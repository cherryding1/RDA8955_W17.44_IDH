...
#if !SPIFFS_READ_ONLY
s32_t spiffs_object_update_name(
    spiffs *fs,
    spiffs_fd *fd,
    spiffs_obj_id obj_id,
    spiffs_page_ix pix,
  ...
  memset(newname, 0, SPIFFS_OBJ_NAME_LEN);
  strncpy((char*)newname, (const char*)name, SPIFFS_OBJ_NAME_LEN);

  res = _spiffs_wr(fs, SPIFFS_OP_T_OBJ_DA | SPIFFS_OP_C_UPDT,
        fd->file_nbr, SPIFFS_PAGE_TO_PADDR(fs, pix) + offsetof(spiffs_page_object_ix_header, name), SPIFFS_OBJ_NAME_LEN, newname);
