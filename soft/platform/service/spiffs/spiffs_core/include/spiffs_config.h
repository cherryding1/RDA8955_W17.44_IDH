...
struct spiffs_t;
extern void sf_fslock_impl(struct spiffs_t *fs);
extern void sf_fsunlock_impl(struct spiffs_t *fs);
#define SPIFFS_LOCK(fs) sf_fslock_impl(fs)
#define SPIFFS_UNLOCK(fs) sf_fsunlock_impl(fs)

// DONT ENABLE CACHE, UNTIL IT IS FULLY TESTED.
