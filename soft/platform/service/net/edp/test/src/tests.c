...
static int vtokeq(char *s, jsmntok_t *t, int numtok, va_list ap);
static void json_test(int (*func)(void), char *name);
static int tokeq(char *s, jsmntok_t *tokens, int numtok, ...);
static int parse(char *s, int status, int numtok, ...);

#define TEST_MALLOC CSW_Malloc
#define TEST_FREE CSW_Free

/* Test runner */
typedef int (*TEST_FUNC)(void);
static void json_test(TEST_FUNC func, char *name) {
	int r = func();
	if (r == 0) {
		test_passed++;
	} else {
		test_failed++;
		TEST_LOG("FAILED: %s (at line %d)\n", name, r);
	}
}
...
static int vtokeq(char *s, jsmntok_t *t, int numtok, va_list ap) {
	if (numtok > 0) {
		...
		for (i = 0; i < numtok; i++) {
			...
			if (t[i].type != type) {
				TEST_LOG("token %d type is %d, not %d\n", i, t[i].type, type);
				return 0;
			}
			if (start != -1 && end != -1) {
				if (t[i].start != start) {
					TEST_LOG("token %d start is %d, not %d\n", i, t[i].start, start);
					return 0;
				}
				if (t[i].end != end ) {
					TEST_LOG("token %d end is %d, not %d\n", i, t[i].end, end);
					return 0;
				}
			}
			if (size != -1 && t[i].size != size) {
				TEST_LOG("token %d size is %d, not %d\n", i, t[i].size, size);
				return 0;
			}
			...
			if (s != NULL && value != NULL) {
				...
				if (strlen(value) != t[i].end - t[i].start ||
					...
					TEST_LOG("token %d value is %.*s, not %s\n", i, t[i].end-t[i].start,
							s+t[i].start, value);
					return 0;
				}
			}
...
static int parse(char *s, int status, int numtok, ...) {
	...
	va_list args;
	jsmn_parser p;
	jsmntok_t *t = TEST_MALLOC(numtok * sizeof(jsmntok_t));

	jsmn_init(&p);
	r = jsmn_parse(&p, s, strlen(s), t, numtok);
	if (r != status) {
		TEST_LOG("status is %d, not %d\n", r, status);
		return 0;
	}
...
static int main_test(void) {
	json_test(test_empty, "test for a empty JSON objects/arrays");
	json_test(test_object, "test for a JSON objects");
	json_test(test_array, "test for a JSON arrays");
	json_test(test_primitive, "test primitive JSON data types");
	json_test(test_string, "test string JSON data types");

	json_test(test_partial_string, "test partial JSON string parsing");
	json_test(test_partial_array, "test partial array reading");
	json_test(test_array_nomem, "test array reading with a smaller number of tokens");
	json_test(test_unquoted_keys, "test unquoted keys (like in JavaScript)");
	json_test(test_input_length, "test strings that are not null-terminated");
	json_test(test_issue_22, "test issue #22");
	json_test(test_issue_27, "test issue #27");
	json_test(test_count, "test tokens count estimation");
	json_test(test_nonstrict, "test for non-strict mode");
	json_test(test_unmatched_brackets, "test for unmatched brackets");
