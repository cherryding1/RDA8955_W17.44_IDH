...
#define MKFUN_PACKET_SAVE_DATA_TYPE2(TYPE, NAME, MED)			\
	...
	EdpPacket* pkg = NULL;						\
									\
	json_obj = cJSON_CreateObject();	\
	if (json_obj == NULL) {printf("json obj err\n");return pkg; } \
...
#define MKFUN_UNPACK_SAVE_DATA_TYPE1(TYPE, NAME, MED)			\
    ...
    *ds_id = (char*)COS_Malloc(strlen(id)+1);				\
...
#define UNPACK_SAVE_DATA_TYPE1_END_STRING				\
    ...
    *value = (char*)COS_Malloc(strlen(valuestring)+1);			\
...
Buffer* NewBuffer()
{
    Buffer* buf = (Buffer*)COS_Malloc(sizeof(Buffer));
    buf->_data = (uint8*)COS_Malloc(sizeof(uint8) * BUFFER_SIZE);
    buf->_write_pos = 0;
    buf->_read_pos = 0;
...
int32 CheckCapacity(Buffer* buf, uint32 len)
{
    ...
    {
        pdata = (uint8*)COS_Malloc(sizeof(uint8) * cap_len);
...
int32 ReadBytes(EdpPacket* pkg, uint8** val, uint32 count)
{
    if (pkg->_read_pos+count > pkg->_write_pos) 
        return -1;
    *val = (uint8*)COS_Malloc(sizeof(uint8) * count);
...
int32 ReadStr(EdpPacket* pkg, char** val)
{
    ...
    if (pkg->_read_pos+len > pkg->_write_pos) 
        return -1;
    /* copy str val */
    *val = (char*)COS_Malloc(sizeof(char) * (len + 1));
    memset(*val, 0, len+1);
...
/* 
 * 瀹氫箟3绉岼SON鏍煎紡鐨勬墦鍖呭拰瑙ｅ寘鍑芥暟锛�
 * 姣忕鏍煎紡閮芥敮鎸乮nt,double鍜宑har*涓夌绫诲瀷
 */
