...
#else
void cg_log_print(int severity, const char *file, int line_n, const char *function, const char *fmt, ...)
{
    char uart_buf[512];
    va_list ap;
	int prefix_length = snprintf(uart_buf, sizeof(uart_buf), "%s:%d:%s ", file, line_n, function);

    va_start (ap, fmt);
    vsnprintf(uart_buf + prefix_length, sizeof(uart_buf) - prefix_length, fmt, ap);
    va_end (ap);
	int nIndex = 0;
switch(severity) {
	case SEV_WARNING:
