...
#include "system/aliyun_iot_ca.h"
#include "aliyun_iot_auth.h"


const static char *IOT_AUTH_DEF_HOST = "http://iot-auth.cn-shanghai.aliyuncs.com/auth/devicename";
static char* s_iot_auth_host[HOST_ADDRESS_LEN + 1] = {0};
void aliyun_iot_set_auth_host(const char* host) {
    memset(s_iot_auth_host, 0, sizeof(s_iot_auth_host));
    strncpy(s_iot_auth_host, host, HOST_ADDRESS_LEN);
}
...
int32_t aliyun_iot_auth(aliot_device_info_pt pdevice_info, aliot_user_info_pt puser_info)
{
    ...
    uint16_t port;
	ALIOT_LOG_INFO("start aliyun iot auth!");
    char* auth_host = s_iot_auth_host[0] == 0 ? IOT_AUTH_DEF_HOST : s_iot_auth_host;
    ...
    if (0 != aliyun_iot_get_id_token(
                ...
                &port))
    {
       ALIOT_LOG_INFO("aliyun auth error return -1");
       return -1;
    }
    ...
    ALIOT_LOG_INFO("aliyun iot auth success!");
    return 0;
}
