...
BOOL cg_upnp_ssdp_server_start(CgUpnpSSDPServer *server)
{
	...
	{
		COS_EVENT ev;
		ev.nEventId = EV_SSDP_SERVER_START_REQ;
		ev.nParam1 = server->recvThread; 
		COS_SendEvent(MOD_SSDP_SERVER, &ev, COS_WAIT_FOREVER, COS_EVENT_PRI_NORMAL);
	}
...
void BAL_SSDP_ServerTask(void* pData)
{
    COS_EVENT ev;

	for(;;)
	{
		COS_WaitEvent(MOD_SSDP_SERVER, &ev, COS_WAIT_FOREVER);
		switch(ev.nEventId)
		{
		case EV_SSDP_SERVER_START_REQ:
			{
				hal_HstSendEvent(SYS_EVENT, 0x08070071);
				cg_upnp_ssdp_server_thread((CgThread*)ev.nParam1);
			}
			break;
		default:
			break;
