...
char *cg_upnp_createuuid(char *uuidBuf, int uuidBufSize)
{
	...
	snprintf(uuidBuf, uuidBufSize, "%s:BA2E90A0-3669-401a-%04x-%04x%04x%04x",
		CG_UPNP_UUID_NAME,
		(int)(time1 & 0xFFFF),
...
char *cg_upnp_getservername(char *buf, int bufSize)
{
	...
	snprintf(buf, bufSize, "sxr/rdamicro UPnP/%s %s/%s", CG_UPNP_VER,CG_CLINK_NAME, CG_CLINK_VER);
#else
	sprintf(buf, "sxr/rdamicro UPnP/%s %s/%s", CG_UPNP_VER,CG_CLINK_NAME, CG_CLINK_VER);
#endif
