...
static int prv_getDisplayHelpLength(command_desc_t * commandArray)
{
    int length = 0;
    int i;
    for (i = 0; commandArray[i].name != NULL; i++)
    {
        length += strlen(commandArray[i].name) + strlen(commandArray[i].shortDesc) + 3;
    }
    return length;
}
                            ...
                            char * buffer)
{
    ...
    if (cmdP == NULL)
    {
        ...
        lwm2m_report_data(HELP_COMMAND"\t"HELP_DESC"\r\n", strlen(HELP_COMMAND"\t"HELP_DESC"\r\n"));
        ...
        for (i = 0 ; commandArray[i].name != NULL ; i++)
        {
            ...
            displayLength = strlen(commandArray[i].name) + strlen(commandArray[i].shortDesc) + 3;
            data = (char *) lwm2m_malloc(displayLength);

            if (data)
            {
                snprintf(data, displayLength, "%s\t%s\r\n", commandArray[i].name, commandArray[i].shortDesc);
                sleep(100);
                lwm2m_report_data(data, displayLength);
                lwm2m_report_data("\r\n", 2);
            }
        }
        lwm2m_report_data("\r\n", 2);
    ...
    else
    {
        ...
        lwm2m_report_data("\r\n", 2);
        lwm2m_report_data(cmdP->longDesc?cmdP->longDesc:cmdP->shortDesc, strlen(cmdP->longDesc?cmdP->longDesc:cmdP->shortDesc) + 2);
        lwm2m_report_data("\r\n", 2);
    }
}
                    ...
                    char * buffer)
{
    ...
    else
    {
        ...
        else
        {
            ...
            lwm2m_report_data(UNKNOWN_CMD_MSG, strlen(UNKNOWN_CMD_MSG));
            lwm2m_report_data("\r\n", 2);
        }
    }
              ...
              int indent)
{
    ...
    for(i= 0 ; i < size ; i++)
    {
        ...
        char *data = NULL;
        data = (char *)lwm2m_malloc(10);
        if (data)
        {
            snprintf(data, 10, "id: %d\r\n", dataP[i].id);
        ...
        fprintf(stream, "type: ");
        lwm2m_report_data("type: ", strlen("type: "));
        ...
        case LWM2M_TYPE_OBJECT:
            ...
            lwm2m_report_data("LWM2M_TYPE_OBJECT\r\n", strlen("LWM2M_TYPE_OBJECT\r\n"));
        ...
        case LWM2M_TYPE_OBJECT_INSTANCE:
            ...
            lwm2m_report_data("LWM2M_TYPE_OBJECT_INSTANCE\r\n", strlen("LWM2M_TYPE_OBJECT_INSTANCE\r\n"));
            dump_tlv(stream, dataP[i].value.asChildren.count, dataP[i].value.asChildren.array, indent + 1);
            break;
        case LWM2M_TYPE_MULTIPLE_RESOURCE:
            fprintf(stream, "LWM2M_TYPE_MULTIPLE_RESOURCE\r\n");
            lwm2m_report_data("LWM2M_TYPE_MULTIPLE_RESOURCE\r\n", strlen("LWM2M_TYPE_MULTIPLE_RESOURCE\r\n"));
            dump_tlv(stream, dataP[i].value.asChildren.count, dataP[i].value.asChildren.array, indent + 1);
            break;
        case LWM2M_TYPE_UNDEFINED:
            fprintf(stream, "LWM2M_TYPE_UNDEFINED\r\n");
            lwm2m_report_data("LWM2M_TYPE_UNDEFINED\r\n", strlen("LWM2M_TYPE_UNDEFINED\r\n"));
            break;
        case LWM2M_TYPE_STRING:
            fprintf(stream, "LWM2M_TYPE_STRING\r\n");
            lwm2m_report_data("LWM2M_TYPE_STRING\r\n", strlen("LWM2M_TYPE_STRING\r\n"));
            print_indent(stream, indent + 1);

            fprintf(stream, "\"%.*s\"\r\n", (int)dataP[i].value.asBuffer.length, dataP[i].value.asBuffer.buffer);

            lwm2m_report_data(dataP[i].value.asBuffer.buffer, (int)dataP[i].value.asBuffer.length);
            lwm2m_report_data("\r\n", 2);
            break;
        case LWM2M_TYPE_OPAQUE:
        ...
        case LWM2M_TYPE_INTEGER:
            ...
            lwm2m_report_data("LWM2M_TYPE_INTEGER: ", strlen("LWM2M_TYPE_INTEGER: "));
            print_indent(stream, indent + 1);
            ...
            value = dataP[i].value.asInteger;
            data = (char *)lwm2m_malloc(10);
            snprintf(data, 10, "%" PRId64, value);
            lwm2m_report_data(data, strlen(data));
        ...
        case LWM2M_TYPE_FLOAT:
            ...
            lwm2m_report_data("LWM2M_TYPE_FLOAT: ", strlen("LWM2M_TYPE_FLOAT: "));
            print_indent(stream, indent + 1);
            fprintf(stream, "%" PRId64, dataP[i].value.asInteger);
        ...
        case LWM2M_TYPE_BOOLEAN:
            ...
            lwm2m_report_data("LWM2M_TYPE_BOOLEAN: ", strlen("LWM2M_TYPE_BOOLEAN: "));

            fprintf(stream, "%s", dataP[i].value.asBoolean ? "true" : "false");

            lwm2m_free(data);

            data = (char *)lwm2m_malloc(strlen(dataP[i].value.asBoolean ? "true" : "false") + 1);
            snprintf(data, strlen(dataP[i].value.asBoolean ? "true" : "false") + 1, "%s", dataP[i].value.asBoolean ? "true" : "false");
            lwm2m_report_data(data, strlen(data));
        ...
        case LWM2M_TYPE_OBJECT_LINK:
            ...
            lwm2m_report_data("LWM2M_TYPE_OBJECT_LINK\r\n", strlen("LWM2M_TYPE_OBJECT_LINK\r\n"));
            break;
        default:
            fprintf(stream, "unknown (%d)\r\n", (int)dataP[i].type);
            ...
            data = (char *)lwm2m_malloc(16);
            snprintf(data, 16, "unknown (%d)\r\n", (int)dataP[i].type);
            lwm2m_report_data(data, strlen(data));
