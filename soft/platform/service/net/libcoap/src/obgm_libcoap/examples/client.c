...
typedef void (*coap_return_handler_t)(void *param,const unsigned char *data, size_t len);

coap_return_handler_t coap_return_handler;
void *coap_return_handler_param;
...
append_to_output(const unsigned char *data, size_t len) {
  ...
  fflush(file);
#else
    if (coap_return_handler != NULL)
        coap_return_handler(coap_return_handler_param,data,len);
#endif
  return 0;
}
...
static void coap_client_dinit()
{
    ...
    max_wait;                   /* global timeout (changed by set_timeout()) */
    
    obs_seconds = 30;          /* default observe time */
    obs_wait = 0;               /* timeout for current subscription */
    total_file_size = 0;
    coap_return_handler = NULL;
    coap_return_handler_param = NULL;
}
...
u32_t coap_client_remain_data() {
    return total_file_size - payload.length;
}

void coap_client_set_returnhander(coap_return_handler_t handler,void *param) {
    coap_return_handler = handler;
    coap_return_handler_param = param;
}
