...
extern UINT8  rfd_XcvRxOn(RFD_WIN_T *Win, INT16 start, INT16* I_Dcoffset,INT16* Q_Dcoffset,UINT8* xcvAnaLevel,UINT8* xcvDigLevel);
#else
...
//----------------------------------------------------------------------
PUBLIC VOID rfd_XcvFrameStart(VOID);
...
PUBLIC VOID rfd_XcvSetXcvMode(bool chorfremod);  // 0 channel mode , 1 frequency mode
PUBLIC VOID rfd_XcvSetOokPower(char power);  // 4 power level
PUBLIC VOID rfd_XcvSetOokChannel(INT16 channel);  // [0-163] max channel is 163
PUBLIC VOID rfd_XcvSetOokData(bool Ookdata);  //Ookdata
PUBLIC VOID rfd_XcvSetXcvTestMode(bool testmod);  // 1 testmode, 0 normal mode
///@} <- End of Xcver API
