...
#include "gpio_i2c.h"
#error "PAS6175 use the hardware I2C"
#else
...
//if(protect ==1)  hal_DbgAssert("pas6175:");
...
PRIVATE VOID camerap_WriteOneReg(UINT8 Addr, UINT8 Data)
{
    ...
    hal_DbgAssert("pas6175: i2c no ack when writing reg 0x%x", Addr);
#else
...
PRIVATE VOID camerap_WriteReg(CAM_REG_T RegWrite)
{
...
#if 1
    while ((HAL_ERR_NO != hal_I2cSendByte(g_camdI2cBusId, 0x6e, RegWrite.Addr, RegWrite.Data)) && (--I2cWritingN == 0))
    {
        //  hal_DbgAssert("pas6175: i2c no ack when writing ");
        hal_HstSendEvent(0xccccdddf);
    }
#else
    ...
    hal_DbgAssert("pas6175: i2c no ack when writing reg 0x%x", RegWrite.Addr);
...
UINT8 test_color = 3;
void PAS6175_Previewqqvga(UINT16 image_target_width, UINT16 image_target_height, UINT8 clk_NP, UINT32 HBlank)
{
}
...
//   PAS6175_Set_Image_Size(image_target_width,image_target_height);
    ...
    PAS6175_Set_Image_Size(240,320);
...
//  if(lpf_value!=0)
    ...
    PAS6175_Set_Para_Effect(0);//CAM_EFFECT_ENC_GRAYSCALE);
...
大delay。*/
void PAS6175_Clkdelay(UINT8 pclk_delay)
{
...
PRIVATE void camerap_PowerOn(CAM_SIZE_T CamOutSize, UINT16 IspOutWidth, UINT16 IspOutHeight, CAM_FORMAT_T Format)
{
    ...
    SXS_TRACE(TSTDOUT, "bf3a01 DRV_CAM:1 Power On, camOutSize %d ispw %d isph %d", CamOutSize, IspOutWidth, IspOutHeight);

    CamConfig.rstActiveH = FALSE;
    CamConfig.pdnActiveH = TRUE;
    CamConfig.dropFrame = FALSE;

    CamConfig.camClkDiv = 5; //6;//7 // must synchronize with VIDREC_AVI_VIDEO_FRAME_RATE
...
#if defined(TP_3GTC_HW_V0_PATCH)
...
PRIVATE void camerap_SetContrastL(UINT32 Contrast)
{
    ...
    {
    case 1:
        camerap_WriteOneReg(0xfe, 0x00); //{0xfe, 0x00, 0xff},
        camerap_WriteOneReg(0xce, 0x70); //{0xce, 0x10, 0xff},
        camerap_WriteOneReg(0xff, 0xff);
        break;
    case 2:
        camerap_WriteOneReg(0xfe, 0x00); //{0xfe, 0x00, 0xff},
        camerap_WriteOneReg(0xce, 0x55); //{0xce, 0x10, 0xff},
        camerap_WriteOneReg(0xff, 0xff);
        break;
    case 3:
        camerap_WriteOneReg(0xfe, 0x00); //{0xfe, 0x00, 0xff},
        camerap_WriteOneReg(0xce, 0x44); //{0xce, 0x10, 0xff},
        camerap_WriteOneReg(0xff, 0xff);
        break;
    case 4:
        camerap_WriteOneReg(0xfe, 0x00); //{0xfe, 0x00, 0xff},
        camerap_WriteOneReg(0xce, 0x33); //{0xce, 0x10, 0xff},
        camerap_WriteOneReg(0xff, 0xff);
        break;
    case 5:
        camerap_WriteOneReg(0xfe, 0x00); //{0xfe, 0x00, 0xff},
        camerap_WriteOneReg(0xce, 0x22); //{0xce, 0x10, 0xff},
        camerap_WriteOneReg(0xff, 0xff); //{0xff, 0xff, 0xff},
        break;
    default:
        break;
    }
...
PRIVATE void camerap_SetBrightnessL(UINT32 Brightness)
{
    ...
    case 1: //0 //-1
        camerap_WriteOneReg(0xfe, 0x01);
        camerap_WriteOneReg(0x04, 0x20);
        camerap_WriteOneReg(0xff, 0xff);
        break;
    case 2: //-2 //-0.5
        camerap_WriteOneReg(0xfe, 0x01);
        camerap_WriteOneReg(0x04, 0x30);
        camerap_WriteOneReg(0xff, 0xff);

        break;
    case 3: //-1 // 0
        camerap_WriteOneReg(0xfe, 0x01);
        camerap_WriteOneReg(0x04, 0x55);
        camerap_WriteOneReg(0xff, 0xff);

        break;
    case 4: //+1 // 1
        camerap_WriteOneReg(0xfe, 0x01);
        camerap_WriteOneReg(0x04, 0x60);
        camerap_WriteOneReg(0xff, 0xff);

        break;
    case 5: //+2//1
        camerap_WriteOneReg(0xfe, 0x01);
        camerap_WriteOneReg(0x04, 0x70);
        camerap_WriteOneReg(0xff, 0xff);
        break;
    default:
        break;
...
{
    SXS_TRACE(TSTDOUT, "bf3a01 DRV_CAM:=== Banding %d||CAM_BANDING_50HZ=%d ||CAM_BANDING_60HZ=%d", iBanding,
              CAM_BANDING_50HZ, CAM_BANDING_60HZ);
    switch (iBanding)
    {
    case CAM_BANDING_50HZ:
        camerap_WriteOneReg(0xfe, 0x01);
        camerap_WriteOneReg(0x07, 0xde);
        //  camerap_WriteOneReg(0x0c,0x30);
        //  camerap_WriteOneReg(0xff,0xff);
        camerap_WriteOneReg(0x0c, 0x2d);

        break;
    case CAM_BANDING_60HZ:
        camerap_WriteOneReg(0xfe, 0x01);
        camerap_WriteOneReg(0x07, 0xdc);
        //  camerap_WriteOneReg(0x0d,0x28);
        //  camerap_WriteOneReg(0xff,0xff);
        camerap_WriteOneReg(0x0d, 0x25);
        break;
    default:
        break;
    }
...
PRIVATE void camerap_SetWhiteBalanceL(CAM_WHITEBALANCE_T WhiteBalance)
{
    ...
    case CAM_WB_DAYLIGHT: /*晴天太阳光*/
        camerap_WriteOneReg(0xfe, 0x00);
        camerap_WriteOneReg(0xa0, 0x40);
        camerap_WriteOneReg(0xb2, 0x01);
        camerap_WriteOneReg(0xb3, 0x11);
        camerap_WriteOneReg(0xb0, 0x14);
        camerap_WriteOneReg(0xb1, 0x34);
        camerap_WriteOneReg(0xff, 0xff);
    ...
    case CAM_WB_TUNGSTEN: /*钨丝灯*/
        ...
        camerap_WriteOneReg(0xb2, 0x01);
        camerap_WriteOneReg(0xb3, 0x11);
        camerap_WriteOneReg(0xb0, 0x2c);
        camerap_WriteOneReg(0xb1, 0x18);
        camerap_WriteOneReg(0xff, 0xff);
...
PRIVATE VOID camerap_SetSpecialEffect(CAM_SPECIALEFFECT_T SpecialEffect)
{
    ...
    case CAM_SPE_NORMAL:
        camerap_WriteOneReg(0xfe, 0x01);
        camerap_WriteOneReg(0x5d, 0x00);
        camerap_WriteOneReg(0x5e, 0x00);
        camerap_WriteOneReg(0x5f, 0x00);
        camerap_WriteOneReg(0xff, 0xff);
        break;

    case CAM_SPE_ANTIQUE:
        camerap_WriteOneReg(0xfe, 0x01);
        camerap_WriteOneReg(0x5d, 0x10);
        camerap_WriteOneReg(0x5e, 0x30);
        camerap_WriteOneReg(0x5f, 0x90);
        camerap_WriteOneReg(0xff, 0xff);
        break;
    ...
    case CAM_SPE_REDISH:
        camerap_WriteOneReg(0xfe, 0x01);
        camerap_WriteOneReg(0x5d, 0x10);
        camerap_WriteOneReg(0x5e, 0x60);
        camerap_WriteOneReg(0x5f, 0xb0);
        camerap_WriteOneReg(0xff, 0xff);
        break;
    ...
    case CAM_SPE_BLACKWHITE:
        camerap_WriteOneReg(0xfe, 0x01);
        camerap_WriteOneReg(0x5d, 0x10);
        camerap_WriteOneReg(0x5e, 0x80);
        camerap_WriteOneReg(0x5f, 0x80);
        camerap_WriteOneReg(0xff, 0xff);
        break;

    case CAM_SPE_NEGATIVE:
        camerap_WriteOneReg(0xfe, 0x01);
        camerap_WriteOneReg(0x5d, 0x40);
        camerap_WriteOneReg(0x5e, 0x00);
        camerap_WriteOneReg(0x5f, 0x00);
        camerap_WriteOneReg(0xff, 0xff);
        break;
    ...
    case CAM_MODE_PREVIEW_STAGE_2:
        ...
        if (cause.fend)
        {
            ...
            else
            {
                ...
                s_contLossDbgCnt--;
                if (!s_contLossDbgCnt)
                    hal_DbgAssert("!!Assert for continuous camera image loss!!");
            }
...
PRIVATE BOOL camerap_CheckProductId()
{
            ...
            hal_CameraReset(FALSE);
    {
        ...
        SXS_TRACE(TSTDOUT, "pas6175: got cam id 0x%x", cameraID);
    ...
    SXS_TRACE(TSTDOUT, "pas6175(0x%02x): read id is 0x%02x", CAM_BY3A01_ID, cameraID);
    ...
    if (cameraID == CAM_BY3A01_ID)
    {
        ...
        hal_HstSendEvent(0xaacc0000 | CAM_BY3A01_ID);
...
PUBLIC BOOL camera_bf3a01_jyj_h1_baili_RegInit(CAM_DRV_T *pCamDrv)
{
    if (camerap_CheckProductId())
    {
        ...
        pCamDrv->camera_SetBanding = camerap_SetBanding;
