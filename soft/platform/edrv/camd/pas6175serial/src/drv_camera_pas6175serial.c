...
PRIVATE VOID  camerap_WriteOneReg( UINT8 Addr, UINT8 Data)
{
    ...
    if(HAL_ERR_NO != hal_I2cSendByte(g_camdI2cBusId,0x40,Addr, Data))
...
#if 1
    if(HAL_ERR_NO != hal_I2cSendByte(g_camdI2cBusId,0x40,RegWrite.Addr, RegWrite.Data))
...
//   hal_HstSendEvent(0x8844009);
    SXS_TRACE(TSTDOUT,"camerap_PowerOnForPreview, camOutSize %d ispw %d isph %d", CamOutSize, IspOutWidth, IspOutHeight);
...
//  camerap_SendRgSeq(RG_InitPowerOnQvgaPas,  (sizeof(RG_InitPowerOnQvgaPas) / sizeof(CAM_REG_T)));

    camerap_SendRgSeq(RG_InitPowerOnQvgaPas240320s,  (sizeof(RG_InitPowerOnQvgaPas240320s) / sizeof(CAM_REG_T)));
...
PRIVATE void camerap_PowerOn(CAM_SIZE_T CamOutSize, UINT16 IspOutWidth, UINT16 IspOutHeight, CAM_FORMAT_T Format)
{
...
#else
    g_camdI2cBusId =HAL_I2C_BUS_ID_1;//tgt_GetCamdConfig()->i2cBusId;
    ...
    {
        PAS6175_Previewqqvga(128,160,8,60);
    }
...
PRIVATE VOID camerap_InteruptHandler(HAL_CAMERA_IRQ_CAUSE_T cause)
{
    ...
    switch (gSensorInfo.cammode)
    {
        ...
        case CAM_MODE_CAPTURE_STAGE_2:
            ...
            s       camerap_WriteOneReg(0xfe,0x00);
            s       camerap_WriteOneReg(0xe5,0x01);
            //end
#else
            if (cause.fend)
            {
...
PRIVATE BOOL camerap_CheckProductId()
{
        ...
        hal_CameraReset(FALSE);
    {
        ...
        hal_HstSendEvent(0x12345600); hal_HstSendEvent(cameraID);
    ...
    SXS_TRACE(TSTDOUT,"pas6175(0x%02x): read id is 0x%02x", CAM_PAS6175_ID, cameraID);
    hal_HstSendEvent(0x12345600); hal_HstSendEvent(cameraID);
    if(cameraID == CAM_PAS6175_ID)
        return TRUE;
    else
        return FALSE;
}
...
PUBLIC BOOL camera_pas6175serial_RegInit(CAM_DRV_T  * pCamDrv)
{
