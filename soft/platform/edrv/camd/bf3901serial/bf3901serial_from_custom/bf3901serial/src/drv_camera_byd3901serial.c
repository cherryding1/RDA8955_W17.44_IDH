...
UINT32 BF3901_Set_Para_Effect(UINT32 iPara)
{
//  UINT8 index = 0;
    switch (iPara)
    {
...
PRIVATE void PAS6175_NightMode(BOOL bEnable)
{
    return TRUE;
    UINT8 Index = 0;
...
PRIVATE UINT32 PAS6175_Set_Para_Banding(UINT32 iPara)
{
    UINT8 Reg_vlaue = 0;
...
//  UINT8 Reg_vlaue = 0;
    ...
    CamConfig.spi_mode = SPI_MODE_MASTER1;// s_gc6113_mode;
    ...
    //g_camdI2cBusId =HAL_I2C_BUS_ID_2;//HAL_I2C_BUS_ID_1 tgt_GetCamdConfig()->i2cBusId;
...
// 240- 320 test ok
    ...
    //camerap_WriteOneReg(0xb9,0xe0);  // test color bar
...
PRIVATE VOID camerap_SetSpecialEffect(CAM_SPECIALEFFECT_T SpecialEffect)
{
    ...
    {
        case CAM_SPE_NORMAL :
            BF3901_Set_Para_Effect(CAM_EFFECT_ENC_NORMAL);
        ...
        case CAM_SPE_GREENISH:
            BF3901_Set_Para_Effect(CAM_EFFECT_ENC_SEPIAGREEN);
            break;

        case CAM_SPE_BLUEISH    :
            BF3901_Set_Para_Effect(CAM_EFFECT_ENC_SEPIABLUE);
            break;

        case CAM_SPE_BLACKWHITE :
            BF3901_Set_Para_Effect(CAM_EFFECT_ENC_GRAYSCALE);
            break;
...
#if 0
{
        ...
        hal_CameraReset(FALSE);
    {
        ...
        //     hal_HstSendEvent(0xcccc6dd);

    }
    hal_CameraClose();
    pmd_EnablePower(PMD_POWER_CAMERA, FALSE);

    SXS_TRACE(TSTDOUT,"pas6175(0x%02x): read id is 0x%02x", CAM_BY3901_ID, cameraID);
    hal_HstSendEvent(0x123456dd);
    hal_HstSendEvent(cameraID);



    if(cameraID == CAM_BY3901_ID)
    {
        hal_HstSendEvent(0xaacc0000|CAM_BY3901_ID); hal_HstSendEvent(0x123456ee);
        return TRUE;
    }
    else
    {
        hal_HstSendEvent(0x123456e5);
        return FALSE;
    }
...
#else
{
    ...
    hal_HstSendEvent(CAM_BY3901_ID);
    //   }
    // Disable Camera Interface Module
    hal_CameraClose();
    // Disable I2C
    ...
    SXS_TRACE(TSTDOUT,"byd3703(0x%02x): read id is 0x%02x", CAM_BY3901_ID, cameraID);
    hal_HstSendEvent(0xaacc0000|cameraID);
    hal_HstSendEvent(CAM_BY3901_ID);

    if(cameraID == CAM_BY3901_ID)
    {
        hal_HstSendEvent(0xaacc0000|CAM_BY3901_ID);
        return TRUE;
    }
    else
    {
...
PUBLIC BOOL camera_bf3901serial_RegInit(CAM_DRV_T  * pCamDrv)
{
