...
PRIVATE HAL_CAMERA_SPI_MODE_E s_spi_mode = SPI_MODE_NO;
...
PRIVATE UINT8 camerap_ReadReg(UINT8 Addr)
{
    ...
    gpio_i2c_read_data(s_i2c_addr, &Addr, 1, &data, 1);
    return data;
#else
...
PRIVATE VOID  camerap_WriteOneReg( UINT8 Addr, UINT8 Data)
{
    ...
    while (i++ < 3)
        if( gpio_i2c_write_data(s_i2c_addr, &Addr, 1, &Data, 1) == TRUE)
            return;

    hal_DbgAssert("sensor: i2c no ack when writing reg 0x%x", Addr);
#else
...
PRIVATE VOID camerap_WriteReg(CAM_REG_T RegWrite)
{
    ...
    UINT8 i = 0;
    while (i++ < 3)
        if( gpio_i2c_write_data(s_i2c_addr, (UINT8*)(&(RegWrite.addr)), 1, &(RegWrite.data), 1) == TRUE)
            return;

    hal_DbgAssert("sensor: i2c no ack when writing reg 0x%x", RegWrite.Addr);
...
PRIVATE VOID camerap_GetId0329(VOID)
{
    UINT16 id= 0;
    hal_TimDelay(163);  //
    camerap_WriteOneReg(0xfc, 0x16);
    camerap_WriteOneReg(0xfc, 0x16);
    hal_TimDelay(163);  //add by Michael
    id = camerap_ReadReg(0x00);
    hal_HstSendEvent(0xee610303); hal_HstSendEvent(id);
}
...
//camerap_GetId0329();
    ...
    {
        hal_HstSendEvent(0xee6103b4); hal_HstSendEvent(CamOutSize);
        camerap_SendRgSeq(RG_ISP_QVGA, (sizeof(RG_ISP_QVGA)/sizeof(CAM_REG_T)));
    }
    else
    {
        hal_HstSendEvent(0xee610309);
...
PRIVATE VOID camerap_InteruptHandler(HAL_CAMERA_IRQ_CAUSE_T cause)
{
    ...
    switch (s_cam_run.camMode)
    {
        case CAM_MODE_PREVIEW_STAGE_1:
            if (cause.fend)
            {
                hal_CameraIrqSetMask(mask);   hal_HstSendEvent(s_cam_run.nPixels<<1);
...
PRIVATE BOOL camerap_CheckProductId()
{
    ...
    hal_HstSendEvent(0xeeee0009);   hal_HstSendEvent(camId);
    SXS_TRACE(TSTDOUT,"gc6103 0x%02x",  camId);
...
PRIVATE VOID camerap_GetImageInfo(CAM_SIZE_T * pcamout,UINT16 *pwidth,UINT16 *pheight)
{
    ...
    hal_HstSendEvent(0xeeee000a);
    if (!pcamout || !pwidth || !pheight)
        return;
    hal_HstSendEvent(0xeeee000b);
...
PUBLIC BOOL camera_gc6103forgc0329_RegInit(CAM_DRV_T  * pCamDrv)
{
