...
#define CAM_GC6123_ID   0xb9

#if defined(VDOREC_MODE_VGA) || defined(VDOREC_MODE_QVGA)
#error "VGA/QVGA recording not supported on GC6123 yet."
#endif

#if defined(_CAM_SPECIAL_GC6123_)
typedef enum
{
    CAM_OUT_NULL = 0,
    CAM_OUT_QQVGA,
...
PRIVATE HAL_CAMERA_SPI_MODE_E s_gc6123_mode = SPI_MODE_NO;
#else
static UINT32 s_prevCount = 0;
PRIVATE HAL_CAMERA_SPI_MODE_E s_gc6123_mode = SPI_MODE_MASTER2_2;
#endif
...
PRIVATE VOID  camerap_WriteOneReg( UINT8 Addr, UINT8 Data)
{
    ...
    hal_DbgAssert("gc6123: i2c no ack when writing reg 0x%x", Addr);
#else
...
PRIVATE VOID camerap_WriteReg(CAM_REG_T RegWrite)
{
    ...
    hal_DbgAssert("gc6123: i2c no ack when writing reg 0x%x", RegWrite.Addr);
...
PRIVATE void camerap_PowerOn(CAM_SIZE_T CamOutSize, UINT16 IspOutWidth, UINT16 IspOutHeight, CAM_FORMAT_T Format)
{
    ...
    CamConfig.camClkDiv = 13;        // 156/6 = 26M
    ...
    CamConfig.spi_mode = s_gc6123_mode;
...
PRIVATE void camerap_SetContrastL(UINT32 Contrast)
{
    ...
    {
        case 1:
            camerap_WriteOneReg(0x83, 0x44);
            break;
        case 2:
            camerap_WriteOneReg(0x83, 0x48);
            break;
        case 3:
            camerap_WriteOneReg(0x83, 0x4b);
            break;
        case 4:
            camerap_WriteOneReg(0x83, 0x50);
            break;
        case 5:
            camerap_WriteOneReg(0x83, 0x54);
            break;
        default: break;
    }
}
...
PRIVATE void camerap_SetWhiteBalanceL(CAM_WHITEBALANCE_T WhiteBalance)
{
    ...
    switch(WhiteBalance)
    {
        case CAM_WB_AUTO:  /*Auto */
            camerap_WriteOneReg(0x22, 0x82);
            break;
        case CAM_WB_DAYLIGHT:   /*晴天太阳光*/
            camerap_WriteOneReg(0x22,0x80);
...
PRIVATE BOOL camerap_CheckProductId()
{
    ...
    CamConfig.pdnActiveH = TRUE;
    CamConfig.dropFrame = FALSE;
    CamConfig.camClkDiv = 13; // 156MHz/15 = 10.4MHz
    ...
    SXS_TRACE(TSTDOUT,"gc6123: start to get the cam id");
    camerap_Delay(20);
    cameraID = camerap_GetId();
    SXS_TRACE(TSTDOUT,"gc6123: got cam id 0x%x", cameraID);
    ...
    SXS_TRACE(TSTDOUT,"gc6123(0x%02x): read id is 0x%02x", CAM_GC6123_ID, cameraID);
    hal_HstSendEvent(0x123456dd); hal_HstSendEvent(cameraID);

    //hal_SetParallelCameraMode();

    if(cameraID == CAM_GC6123_ID)
    {
        hal_HstSendEvent(0xaacc0000|CAM_GC6123_ID);
        return TRUE;
    }
    else
    {
...
PUBLIC BOOL camera_gc6123_RegInit(CAM_DRV_T  * pCamDrv)
{
