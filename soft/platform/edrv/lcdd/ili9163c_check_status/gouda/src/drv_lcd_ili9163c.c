...
#if 0
    LCD_nRESET = 1;
    sxr_Sleep(1); // Delay 1ms
    LCD_nRESET = 0;
    sxr_Sleep(10); // Delay 10ms // This delay time is necessary
    LCD_nRESET = 1;
    sxr_Sleep(100); // Delay 50 ms
#endif
//************* Start Initial Sequence **********//
    LCM_WR_CMD(0x11); //Exit Sleep
//sxr_Sleep(20);
    sxr_Sleep(500 MS_WAITING);; //Delay 120ms
    ...
    LCM_WR_CMD(0xE0);
    main_Write_DATA(0x3F);//p1
    main_Write_DATA(0x25);//p2
    main_Write_DATA(0x22);//p3
    main_Write_DATA(0x2F);//p4
    main_Write_DATA(0x28);//p5
    main_Write_DATA(0x0C);//p6
    main_Write_DATA(0x4F);//p7
    main_Write_DATA(0xB7);//p8
    main_Write_DATA(0x3C);//p9
    main_Write_DATA(0x19);//p10
    main_Write_DATA(0x16);//p11
    main_Write_DATA(0x1E);//p12
    main_Write_DATA(0x02);//p13
    main_Write_DATA(0x01);//p14
    main_Write_DATA(0x00);//p15
    ...
    LCM_WR_CMD(0xE1);
    main_Write_DATA(0x00);//p1
    main_Write_DATA(0x1A);//p2
    main_Write_DATA(0x1D);//p3
    main_Write_DATA(0x10);//p4
    main_Write_DATA(0x17);//p5
    main_Write_DATA(0x13);//p6
    main_Write_DATA(0x30);//p7
    main_Write_DATA(0x48);//p8
    main_Write_DATA(0x43);//p9
    main_Write_DATA(0x06);//p10
    main_Write_DATA(0x19);//p11
    main_Write_DATA(0x21);//p12
    main_Write_DATA(0x3D);//p13
    main_Write_DATA(0x3E);//p14
    main_Write_DATA(0x3F);//p15
...
PRIVATE  BOOL lcddp_CheckLCDStatus(VOID)
{
    UINT16 lcdStatus=0;
    UINT16 lcdStatus2=0;
    unsigned char lcdStatus1[5];

    hal_GoudaReadData(0x09,lcdStatus1,5);
    lcdStatus=((((lcdStatus1[2])&0xff)<<8)|((lcdStatus1[3])&0xff));

    hal_GoudaReadData(0x0a,&lcdStatus1,2);
    lcdStatus2=((((lcdStatus1[0])&0xff)<<8)|((lcdStatus1[1])&0xff));
    /*
        hal_HstSendEvent(0x77778888);
        hal_HstSendEvent(lcdStatus);

        hal_HstSendEvent(productId1[0]);
        hal_HstSendEvent(productId1[1]);
        hal_HstSendEvent(productId1[2]);
        hal_HstSendEvent(productId1[3]);
        hal_HstSendEvent(productId1[4]);
    */
    /*
        hal_HstSendEvent(0x77778888);
        hal_HstSendEvent(lcdStatus);
        hal_HstSendEvent(lcdStatus2);
    */
    if(((lcdStatus&0xFFFF) == 0x5304 || (lcdStatus&0xFFFF) == 0x0000) && lcdStatus2 == 0x009c)
        return TRUE;
    else
        return FALSE;
}
