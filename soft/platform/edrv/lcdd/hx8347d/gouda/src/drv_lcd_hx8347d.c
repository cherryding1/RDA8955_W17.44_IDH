...
PRIVATE VOID lcddp_Init(VOID)
{
    ...
    LCM_WR_REG(0xEC,0x0C); //STBA[15:8]
    LCM_WR_REG(0xED,0xC4); //STBA[7:0]
    LCM_WR_REG(0xE8,0x40); //OPON[7:0]
    LCM_WR_REG(0xE9,0x38); //OPON1[7:0]
    LCM_WR_REG(0xF1,0x01); //OTPS1B
    LCM_WR_REG(0xF2,0x10); //GEN
    LCM_WR_REG(0x27,0xA3); //

    //Gamma 2.2 Setting
    LCM_WR_REG(0x40,0x01); //
    LCM_WR_REG(0x41,0x00); //
    LCM_WR_REG(0x42,0x00); //
    LCM_WR_REG(0x43,0x10); //
    LCM_WR_REG(0x44,0x0E); //
    LCM_WR_REG(0x45,0x24); //
    LCM_WR_REG(0x46,0x04); //
    LCM_WR_REG(0x47,0x50); //
    LCM_WR_REG(0x48,0x02); //
    LCM_WR_REG(0x49,0x13); //
    LCM_WR_REG(0x4A,0x19); //
    LCM_WR_REG(0x4B,0x19); //
    LCM_WR_REG(0x4C,0x16); //

    LCM_WR_REG(0x50,0x1B); //
    LCM_WR_REG(0x51,0x31); //
    LCM_WR_REG(0x52,0x2F); //
    LCM_WR_REG(0x53,0x3F); //
    LCM_WR_REG(0x54,0x3F); //
    LCM_WR_REG(0x55,0x3E); //
    LCM_WR_REG(0x56,0x2F); //
    LCM_WR_REG(0x57,0x7B); //
    LCM_WR_REG(0x58,0x09); //
    LCM_WR_REG(0x59,0x06); //
    LCM_WR_REG(0x5A,0x06); //
    LCM_WR_REG(0x5B,0x0C); //
    ...
    LCM_WR_REG(0x1A,0x01); //BT (VGH~15V,VGL~-10V,DDVDH~5V)
    LCM_WR_REG(0x24,0x2F); //VMH(VCOM High voltage ~3.2V)
    LCM_WR_REG(0x25,0x57); //VML(VCOM Low voltage -1.2V)
    //****VCOM offset**///
    LCM_WR_REG(0x23,0x88); //for Flicker adjust //can reload from OTP
    //Power on Setting
    LCM_WR_REG(0x18,0x36); //I/P_RADJ,N/P_RADJ, Normal mode 75Hz
    ...
    //SET PANEL
    LCM_WR_REG(0x36,0x00); //SS_P, GS_P,REV_P,BGR_P
    //Display ON Setting
...
{
    static char hx8347d_id_str[] = "hx8347d\n";
    return hx8347d_id_str;
}
PRIVATE BOOL lcddp_CheckProductId()
{
    ...
    hal_GoudaClose();
    SXS_TRACE(TSTDOUT, "hx8347d(0x%x): lcd read id is 0x%x ", LCD_HX8347D_ID, productId);

    hal_HstSendEvent(0x1cd08347);
    hal_HstSendEvent(productId);

    if(productId == LCD_HX8347D_ID)
        return TRUE;
    else
        return FALSE;
}
...
// ============================================================================
// lcdd_hx8347d_RegInit
// ----------------------------------------------------------------------------
/// register the right lcd driver, according to lcddp_CheckProductId
/// @return #TRUE, #FALSE
// ============================================================================
PUBLIC BOOL  lcdd_hx8347d_RegInit(LCDD_REG_T *pLcdDrv)
{
