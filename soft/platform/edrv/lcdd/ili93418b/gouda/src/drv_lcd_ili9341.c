...
#define  LCD_SPI_ILI9341_BIT8_ID        0x9341
...
//LCD_nRESET = 1;
    ...
    sxr_Sleep(70);
    // Delay 1ms
    //LCM_WR_CMD(0x13); //normal mode
    LCM_WR_CMD(0x20); //inversion off
...
//driver timing control A
    ...
    LCM_WR_CMD(0xCF);
    LCM_WR_DAT(0x00);
    LCM_WR_DAT(0xC1);//99->C1
    LCM_WR_DAT(0X30);

    //set power on seq ctl
    LCM_WR_CMD(0xED);
    LCM_WR_DAT(0x67);
    LCM_WR_DAT(0x03);
    LCM_WR_DAT(0X12);
...
//power control 1
    ...
    LCM_WR_CMD(0xB6);
    LCM_WR_DAT(0x0A);
    LCM_WR_DAT(0x82);//bit7-bit4:REV,GS,SS,SM
    //LCM_WR_DAT(0xb2);//bit7-bit4:REV,GS,SS,SM

    //memory access control
    LCM_WR_CMD(0x36);
//  LCM_WR_DAT(0x48);//Y,X,V,L,bgr,mh,00
    LCM_WR_DAT(0x88);//Y,X,V,L,bgr,mh,00 rotate 180
    //set pixel to rgb565
    LCM_WR_CMD(0x3A);
    LCM_WR_DAT(0x55);
...
void ILI9341_BOE2_4_Initial(void)
{
...
PRIVATE BOOL lcddp_CheckProductId()
{
    ...
    hal_GoudaClose();
    SXS_TRACE(TSTDOUT, "ili9341(0x%x): lcd read id is 0x%x ", LCD_SPI_ILI9341_BIT8_ID, productId);

    hal_HstSendEvent(0x1cd09341);
    hal_HstSendEvent(productId);

    if(productId == LCD_SPI_ILI9341_BIT8_ID)
        return TRUE;
    else
        return FALSE;
}
...
/// @return #TRUE, #FALSE
// ============================================================================
PUBLIC BOOL  lcdd_ili93418b_RegInit(LCDD_REG_T *pLcdDrv)
{
