...
//************* Start Initial Sequence **********//
    ...
    LCM_WR_DAT (0x1D); //VRH[5:0]
    LCM_WR_CMD(0xC1); //Power control
    LCM_WR_DAT (0x12); //SAP[2:0];BT[3:0]
    LCM_WR_CMD(0xC5); //VCM control
    LCM_WR_DAT (0x30);
    LCM_WR_DAT (0x3D);//3//3F
    LCM_WR_CMD(0xC7); //VCM control
    LCM_WR_DAT (0x8E);//0x9e
    LCM_WR_CMD(0x3A); // Memory Access Control
    LCM_WR_DAT (0x55);
...
PRIVATE BOOL lcddp_CheckProductId()
{
    ...
    hal_GoudaClose();
    SXS_TRACE(TSTDOUT, "ili9341(0x%x): lcd read id is 0x%x ", LCD_ILI9341_ID, productId);
