...
PRIVATE VOID lcddp_Init(VOID)
{
    ...
    LCM_WR_DAT(0x01);
    LCM_WR_DAT(0x10);
    LCM_WR_DAT(0x00);//80--spi_2wire_mode,00
    ...
    LCM_WR_DAT(0x00);
    LCM_WR_DAT(0x11); //0x12  //  0x10 1/1   0x11 1/2  0x12  1/4   0x14   1/8
    LCM_WR_CMD(0xec);
    LCM_WR_DAT(0x33);
    LCM_WR_DAT(0x16);
    LCM_WR_DAT(0x16);
...
PRIVATE VOID lcddp_BlitRoi2Win(CONST HAL_GOUDA_WINDOW_T* pRoi, CONST HAL_GOUDA_WINDOW_T* pActiveWin)
{
    ...
    if(g_lcddRotate)
    {
        LCM_WR_CMD(0x2a);    //Set Page Address
        LCM_WR_DAT((pActiveWin->tlPX>>8)&0x00ff);
        LCM_WR_DAT(pActiveWin->tlPX);
        LCM_WR_DAT((pActiveWin->brPX>>8)&0x00ff);
        LCM_WR_DAT(pActiveWin->brPX);

        LCM_WR_CMD(0x2b);    //Set Column Address
        LCM_WR_DAT(0x00);
    ...
    else
    {
        ...
        //hal_HstSendEvent(pActiveWin->brPX);
        //hal_HstSendEvent(pActiveWin->brPY);
        //LCM_WR_CMD(0x2c); //WRITE ram Data display
    }
...
PRIVATE BOOL lcddp_SetDirRotation(VOID)
{
    ...
    #else
    #if defined(__NV3029F_CMO_LCD_PARA__)||defined(__NV3029F_IVO_LCD_PARA__)||defined(__NV3029F_GKW_YUD_LCD_PARA__)
...
PRIVATE BOOL lcddp_CheckProductId()
{
    ...
    hal_GoudaClose();
     ...
     hal_HstSendEvent(0x30297700);
     hal_HstSendEvent(productId);
    SXS_TRACE(TSTDOUT, "nv3029f(0x%x): lcd read id is 0x%x ", LCD_nv3029f_ID, productId);


    if(productId == LCD_nv3029f_ID)
    {
...
PRIVATE LCDD_ERR_T lcdd_change_spi_frequency(BOOL high)
{
#ifdef USE_SPI_LCD
    if(high)
    {
        hal_GoudaSetSpiFreq(LCDD_SPI_CAM_PREV_FREQ);
    }
    else
    {
        hal_GoudaSetSpiFreq(LCDD_SPI_FREQ);
...
{
    static char nv3029f_id_str[] = "nv3029f\n";
    return nv3029f_id_str;
}
// ============================================================================
// lcdd_nv3029f_RegInit
// ----------------------------------------------------------------------------
/// register the right lcd driver, according to lcddp_CheckProductId
/// @return #TRUE, #FALSE
// ============================================================================
PUBLIC BOOL lcdd_nv3029f_RegInit(LCDD_REG_T *pLcdDrv)
{
...
//
        ...
        pLcdDrv->lcdd_ChangeLcdSpiFreq=lcdd_change_spi_frequency;
