...
#endif
#define  LCD_ILI9225G8B_ID      0x9226
...
PRIVATE VOID lcddp_Init(VOID)
{
    ...
    LCM_WR_REG(0x000F, 0x0901);

    //*************Power On sequence ****************//
    sxr_Sleep(50 MS_WAITING);                         // Delay 50ms
    LCM_WR_REG(0x0010, 0x0000);
    LCM_WR_REG(0x0011, 0x1B41); //The register setting is suitable for VCI=2.8V
    sxr_Sleep(50 MS_WAITING);                         // Delay 50ms
    LCM_WR_REG(0x0012, 0x200E);//The register setting is suitable for VCI=2.8V
    LCM_WR_REG(0x0013, 0x0052); //The register setting is suitable for VCI=2.8V
    LCM_WR_REG(0x0014, 0x4B5C); //The register setting is suitable for VCI=2.8V
    //------------- Set GRAM area ------------------//
...
PRIVATE VOID lcddp_BlitRoi2Win(CONST HAL_GOUDA_WINDOW_T* pRoi, CONST HAL_GOUDA_WINDOW_T* pActiveWin)
{
...
#ifdef LCD_HORIZON_DISPLAY
    LCM_WR_REG(0x0037,LCDD_DISP_Y-1-pActiveWin->brPY);//Window Horizontal RAM Address Start
    LCM_WR_REG(0x0036,LCDD_DISP_Y-1-pActiveWin->tlPY);//Window Horizontal RAM Address End
    LCM_WR_REG(0x0039,pActiveWin->tlPX);//Window Vertical RAM Address Start
    LCM_WR_REG(0x0038,pActiveWin->brPX);//Window Vertical RAM Address End

    LCM_WR_REG(0x0020,LCDD_DISP_Y-1-pActiveWin->tlPY);
    LCM_WR_REG(0x0021,pActiveWin->tlPX);
    ...
    else
    {
        ...
        // directly for configuring layers
#ifdef LCM_Y_MIRROR_DISPLAY
        LCM_WR_REG(0x0036,LCDD_DISP_X-1-pActiveWin->tlPX);//Window Horizontal RAM Address Start
        LCM_WR_REG(0x0037,LCDD_DISP_X-1-pActiveWin->brPX);//Window Horizontal RAM Address End
        LCM_WR_REG(0x0038,LCDD_DISP_Y-1-pActiveWin->tlPY);//Window Vertical RAM Address Start
        LCM_WR_REG(0x0039,LCDD_DISP_Y-1-pActiveWin->brPY);//Window Vertical RAM Address End
        LCM_WR_REG(0x0020,LCDD_DISP_X-1-pActiveWin->tlPX);
        LCM_WR_REG(0x0021,LCDD_DISP_Y-1-pActiveWin->tlPY);
#else
...
{
    static char ili9225g8b_id_str[] = "ili9225g8b\n";
    return ili9225g8b_id_str;
}
PRIVATE BOOL lcddp_CheckProductId()
{
    UINT16 productId=0;
#ifdef USE_SPI_LCD
    UINT16 productId_1=0;
#endif
...
#ifdef USE_SPI_LCD
    LCM_WR_REG(0x0066, 0x0001);
    hal_GoudaReadReg(0x00, &productId);
    hal_GoudaReadReg(0x00, &productId);
    hal_HstSendEvent(productId);

    hal_GoudaReadReg(0x00, &productId_1);
    hal_GoudaReadReg(0x00, &productId_1);
    hal_GoudaReadReg(0x00, &productId_1);
    hal_HstSendEvent(productId_1);
    productId=productId+(productId_1>>8);
#else

    hal_GoudaReadReg(0x00, &productId);
#endif
    hal_GoudaClose();
    hal_HstSendEvent(0x1cd09226);
    hal_HstSendEvent(productId);

    SXS_TRACE(TSTDOUT, "ili9225b8b(0x%x): lcd read id is 0x%x ", LCD_ILI9225G8B_ID, productId);

#ifdef LCD_NO_PRODUCT_ID
    hal_HstSendEvent(0xccc09226);
    return TRUE;
#else
    if(productId == LCD_ILI9225G8B_ID)
        return TRUE;
    else
        return FALSE;
#endif
...
/// @return #TRUE, #FALSE
// ============================================================================
PUBLIC BOOL  lcdd_ili9225g8b_RegInit(LCDD_REG_T *pLcdDrv)
{
