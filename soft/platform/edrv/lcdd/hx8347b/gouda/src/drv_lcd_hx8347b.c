...
PRIVATE VOID lcddp_Init(VOID)
{
    ...
    LCM_WR_REG(0x00E5,0x78F0); // set SRAM internal timing
    ...
    LCM_WR_REG(0x0011,0x0227); // Set DC1[2:0],DC0[2:0],VC[2:0]
    sxr_Sleep(50 MS_WAITING);
    LCM_WR_REG(0x0012,0x008A); // External reference voltage= Vci;
    sxr_Sleep(50 MS_WAITING);
    LCM_WR_REG(0x0013,0x1A00); // VDV[4:0] for VCOM amplitude
    LCM_WR_REG(0x0029,0x000E); // VCM[5:0] for VCOMH
    LCM_WR_REG(0x002B,0x000B); // Set Frame Rate
    sxr_Sleep(50 MS_WAITING);
    ...
    LCM_WR_REG(0x0031,0x0604);
    LCM_WR_REG(0x0032,0x0305);
    LCM_WR_REG(0x0035,0x0000);
    LCM_WR_REG(0x0036,0x0C09);
    LCM_WR_REG(0x0037,0x0204);
    LCM_WR_REG(0x0038,0x0301);
    LCM_WR_REG(0x0039,0x0707);
    LCM_WR_REG(0x003C,0x0000);
    LCM_WR_REG(0x003D,0x0A0A);
    //------------------ Set GRAM area ---------------//
...
PRIVATE LCDD_ERR_T lcddp_WakeUp(VOID)
{
    ...
    LCM_WR_REG(0x0010,0x0080); // SAP,BT[3:0],AP,DSTB,SLP
    LCM_WR_REG(0x0011,0x0000); // DC1[2:0],DC0[2:0],VC[2:0]
    LCM_WR_REG(0x0012,0x0000); // VREG1OUT voltage
    LCM_WR_REG(0x0013,0x0000); // VDV[4:0] for VCOM amplitude
    LCM_WR_REG(0x0007,0x0001);
    ...
    sxr_Sleep(50 MS_WAITING);
    LCM_WR_REG(0x0012,0x008A); //Inernal reference voltage =Vci;
    sxr_Sleep(50 MS_WAITING);
    LCM_WR_REG(0x0013,0x1A00); // VDV[4:0] for VCOM amplitude
    LCM_WR_REG(0x0029,0x000E); // VCM[5:0] for VCOMH
    sxr_Sleep(50 MS_WAITING);
...
PRIVATE VOID lcddp_BlitRoi2Win(CONST HAL_GOUDA_WINDOW_T* pRoi, CONST HAL_GOUDA_WINDOW_T* pActiveWin)
{
    ...
    if(g_lcddRotate)
    {
        ...
        LCM_WR_REG(0x0051,pActiveWin->brPY);//Window Horizontal RAM Address End
        LCM_WR_REG(0x0052,LCDD_DISP_Y-1-pActiveWin->brPX);//Window Vertical RAM Address Start
        LCM_WR_REG(0x0053,LCDD_DISP_Y-1-pActiveWin->tlPX);//Window Vertical RAM Address End
...
{
    static char hx8347_id_str[] = "HX8347B\n";
    return hx8347_id_str;
}
PRIVATE BOOL lcddp_CheckProductId()
{
    ...
    hal_GoudaClose();
    hal_HstSendEvent(0x01cd01cd);
    hal_HstSendEvent(productId);
    SXS_TRACE(TSTDOUT, "HX8347B(0x%x): lcd read id is 0x%x ", LCD_HX8347B_ID, productId);

    if(productId == LCD_HX8347B_ID)
        return TRUE;
    else
        return FALSE;
}
...
// ============================================================================
// lcdd_hx8347b_RegInit
// ----------------------------------------------------------------------------
/// register the right lcd driver, according to lcddp_CheckProductId
/// @return #TRUE, #FALSE
// ============================================================================
PUBLIC BOOL  lcdd_hx8347b_RegInit(LCDD_REG_T *pLcdDrv)
{
