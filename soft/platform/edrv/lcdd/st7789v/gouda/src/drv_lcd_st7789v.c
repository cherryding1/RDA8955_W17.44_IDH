...
#define  LCD_ST7789v_ID       0X5285//0X8552//0x4093
// =============================================================================
//  GLOBAL VARIABLES
// =============================================================================
...
PRIVATE VOID lcddp_BlitRoi2Win(CONST HAL_GOUDA_WINDOW_T* pRoi, CONST HAL_GOUDA_WINDOW_T* pActiveWin)
{
    ...
    /* #if 1
      ...
      LCM_WR_CMD(0x2a);
            ...
            LCM_WR_CMD(0x2b);
            LCM_WR_DATA((pActiveWin->tlPY&0xff00)>>8);
    ...
    if(g_lcddRotate)
    {
        ...
        LCM_WR_DATA(pActiveWin->tlPX>>8);
        LCM_WR_DATA(pActiveWin->tlPX&0x00ff);
        LCM_WR_DATA(pActiveWin->brPX>>8);
        LCM_WR_DATA(pActiveWin->brPX&0x00ff);


        LCM_WR_CMD(0x2b);    //Set Page Address
        LCM_WR_DATA(pActiveWin->tlPY>>8);
        LCM_WR_DATA(pActiveWin->tlPY&0x00ff);
        LCM_WR_DATA(pActiveWin->brPY>>8);
        LCM_WR_DATA(pActiveWin->brPY&0x00ff);
    }
    else
    {
...
PRIVATE BOOL lcddp_CheckProductId()
{
    ...
    hal_GoudaReadRegLength(0x04, &productId,4);
    ...
    SXS_TRACE(TSTDOUT, "gc9301ds(0x%x): lcd read id is 0x%x ", LCD_ST7789v_ID, productId);

    if(productId[1] == LCD_ST7789v_ID)
        return TRUE;
    else
        return FALSE;
}
...
/// @return #TRUE, #FALSE
// ============================================================================
PUBLIC BOOL  lcdd_st7789v_RegInit(LCDD_REG_T *pLcdDrv)
{
