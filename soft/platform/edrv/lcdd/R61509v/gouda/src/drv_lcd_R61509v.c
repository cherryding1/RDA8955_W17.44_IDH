...
PRIVATE LCDD_ERR_T lcddp_Sleep(VOID)
{
    ...
    sxr_Sleep(20 MS_WAITING); // After Inter-MicroP Program (load OTP)
...
PRIVATE VOID lcddp_BlitRoi2Win(CONST HAL_GOUDA_WINDOW_T* pRoi, CONST HAL_GOUDA_WINDOW_T* pActiveWin)
{
    ...
    if(1)
    {
        ...
        // hal_HstSendEvent(0x33440005);
        //  hal_HstSendEvent(pActiveWin->tlPX);//0x50
        //  hal_HstSendEvent(pActiveWin->brPX);//0xcf
        //  hal_HstSendEvent(pActiveWin->tlPY);//0x79
        //  hal_HstSendEvent(pActiveWin->brPY);//0xb4

        //Window Horizontal RAM Address Start
        LCDD_BUILD_CMD_WR_REG(cmd,0,0x210,pActiveWin->tlPX);
        //Window Horizontal RAM Address End
        LCDD_BUILD_CMD_WR_REG(cmd,2,0x211,pActiveWin->brPX);
        //Window Vertical RAM Address Start
        LCDD_BUILD_CMD_WR_REG(cmd,4,0x212,pActiveWin->tlPY);
        //Window Vertical RAM Address End
        LCDD_BUILD_CMD_WR_REG(cmd,6,0x213,pActiveWin->brPY);

        //Start point
        LCDD_BUILD_CMD_WR_REG(cmd,8,0x200,pActiveWin->tlPX);
        LCDD_BUILD_CMD_WR_REG(cmd,10,0x201,pActiveWin->tlPY);
    }
    else
    {
        //Window Horizontal RAM Address Start
        LCDD_BUILD_CMD_WR_REG(cmd,0,0x210,LCDD_DISP_X-1-pActiveWin->brPX);
        //Window Horizontal RAM Address End
        LCDD_BUILD_CMD_WR_REG(cmd,2,0x211,LCDD_DISP_X-1-pActiveWin->tlPX);
        //Window Vertical RAM Address Start
        LCDD_BUILD_CMD_WR_REG(cmd,4,0x212,LCDD_DISP_Y-1-pActiveWin->brPY);
        //Window Vertical RAM Address End
        LCDD_BUILD_CMD_WR_REG(cmd,6,0x213,LCDD_DISP_Y-1-pActiveWin->tlPY);

        //Start point
        LCDD_BUILD_CMD_WR_REG(cmd,8,0x200,LCDD_DISP_X-1-pActiveWin->tlPX);
        LCDD_BUILD_CMD_WR_REG(cmd,10,0x201,LCDD_DISP_Y-1-pActiveWin->tlPY);
    }

    // Send command after which the data we sent
    // are recognized as pixels.
    LCDD_BUILD_CMD_WR_CMD(cmd,12,0x202);
...
{
    static char R61509v_id_str[] = "R61509v\n";
    return R61509v_id_str;
}
PRIVATE BOOL lcddp_CheckProductId()
{
    ...
    SXS_TRACE(TSTDOUT, "R61509v(0x%x): lcd read id is 0x%x ", LCD_R61509v_ID, productId);
    hal_HstSendEvent(0x11110008);
...
// ============================================================================
// lcdd_R61509v_RegInit
// ----------------------------------------------------------------------------
/// register the right lcd driver, according to lcddp_CheckProductId
/// @return #TRUE, #FALSE
// ============================================================================
PUBLIC BOOL lcdd_R61509v_RegInit(LCDD_REG_T *pLcdDrv)
{
