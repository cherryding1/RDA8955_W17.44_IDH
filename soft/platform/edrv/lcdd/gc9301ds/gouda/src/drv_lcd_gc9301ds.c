...
PRIVATE VOID lcddp_Init(VOID)
{
    ...
    LCM_WR_CMD(0xfe);
    LCM_WR_CMD(0xef); //analog voltage adjust enable
    LCM_WR_CMD(0xa3); //set Frame rate
    LCM_WR_DATA (0x82);
    LCM_WR_CMD(0xa4); //set VRH[3:0]
    LCM_WR_DATA (0x8c);
    LCM_WR_CMD(0xff); //set VDV[4:0]
    LCM_WR_DATA (0x0b);
    LCM_WR_CMD(0xfd);
    LCM_WR_DATA (0x0c); // set VCM[5:0]
    LCM_WR_CMD(0xee);
    LCM_WR_DATA (0xc0);
    LCM_WR_CMD(0xf0); //set Gamma
    LCM_WR_DATA (0x20);
    LCM_WR_CMD(0xf9);
    LCM_WR_DATA (0x75);
...
{
    static char gc9301ds_id_str[] = "gc9301ds\n";
    return gc9301ds_id_str;
}
...
PRIVATE UINT16 lcdd_get_lcd_id(void)
{
    ...
    hal_GoudaReadRegLength(0xd3, &productId,4);
...
PRIVATE BOOL lcddp_CheckProductId()
{
    ...
    hal_GoudaReadRegLength(0xd3, &productId,4);
    hal_HstSendEvent(0xcc667756);
    hal_HstSendEvent(productId[0]); hal_HstSendEvent(productId[1]);

    hal_GoudaClose();

    SXS_TRACE(TSTDOUT, "gc9301ds(0x%x): lcd read id is 0x%x ", LCD_GC9301_ID, productId[1]);

    if(productId[1] == LCD_GC9301_ID)
        return TRUE;
    else
        return FALSE;
}
...
/// @return #TRUE, #FALSE
// ============================================================================
PUBLIC BOOL  lcdd_gc9301ds_RegInit(LCDD_REG_T *pLcdDrv)
{
