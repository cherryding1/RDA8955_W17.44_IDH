...
extern U16          simulator_Merge_buffer[176 * 220];
// =============================================================================
...
#define WriteCommand_Reg(addr,data)   { WriteCommand_Addr(addr); WriteCommand_Data((data>>8));WriteCommand_Data(data);}
...
PRIVATE VOID lcddp_Init(VOID)
{
    ...
    //************* Start Initial Sequence **********//
    WriteCommand_Reg(0x00D0, 0x0003);
    WriteCommand_Reg(0x00EB, 0x0B00);
    WriteCommand_Reg(0x00EC, 0x004F);
    WriteCommand_Reg(0x00C7, 0x030F);

    WriteCommand_Reg(0x0001, 0x011C);
    WriteCommand_Reg(0x0002, 0x0100);
    WriteCommand_Reg(0x0003, 0x1030);
    WriteCommand_Reg(0x0007, 0x0000);
    WriteCommand_Reg(0x0008, 0x0808);
    WriteCommand_Reg(0x000F, 0x0B01);

    WriteCommand_Reg(0x0010, 0x0000);
    WriteCommand_Reg(0x0011, 0x1B41); //The register setting is suitable for VCI=2.8V
    WriteCommand_Reg(0x0012, 0x200E);//The register setting is suitable for VCI=2.8V
    WriteCommand_Reg(0x0013, 0x0052); //The register setting is suitable for VCI=2.8V
    WriteCommand_Reg(0x0014, 0x626C); // 626C. The register setting is suitable for VCI=2.8V

    WriteCommand_Reg(0x0030, 0x0000);
    WriteCommand_Reg(0x0031, 0x00DB);
    WriteCommand_Reg(0x0032, 0x0000);
    WriteCommand_Reg(0x0033, 0x0000);
    WriteCommand_Reg(0x0034, 0x00DB);
    WriteCommand_Reg(0x0035, 0x0000);
    WriteCommand_Reg(0x0036, 0x00AF);
    WriteCommand_Reg(0x0037, 0x0000);
    WriteCommand_Reg(0x0038, 0x00DB);
    WriteCommand_Reg(0x0039, 0x0000);

    WriteCommand_Reg(0x0050, 0x0000);
    WriteCommand_Reg(0x0051, 0x0400);
    WriteCommand_Reg(0x0052, 0x0A01);
    WriteCommand_Reg(0x0053, 0x0701);
    WriteCommand_Reg(0x0054, 0x010A);
    WriteCommand_Reg(0x0055, 0x0004);
    WriteCommand_Reg(0x0056, 0x0000);
    WriteCommand_Reg(0x0057, 0x0107);
    WriteCommand_Reg(0x0058, 0x0F00);
    WriteCommand_Reg(0x0059, 0x000F);;

    WriteCommand_Reg(0x0020, 0x0000);
    WriteCommand_Reg(0x0021, 0x0000);
    WriteCommand_Reg(0x0007, 0x1017);
...
PRIVATE LCDD_ERR_T lcddp_SetPixel16(UINT16 x, UINT16 y, UINT16 pixelData)
{
    ...
    else
    {
        ...
        WriteCommand_Reg(0x0037,0x0000);
        WriteCommand_Reg(0x0036,LCDD_DISP_X - x - 1);
        WriteCommand_Reg(0x0039,0x0000);
        WriteCommand_Reg(0x0038,LCDD_DISP_Y - y - 1);

        WriteCommand_Reg(0x0022, pixelData);
...
//    hal_HstSendEvent(0x88855509);
    ...
    if(g_lcddRotate)
    {
        ...
        //Window Horizontal RAM Address Start
        WriteCommand_Reg(0x0037,pActiveWin->tlPY);
        //Window Horizontal RAM Address End
        WriteCommand_Reg(0x0036,pActiveWin->brPY);
        //Window Vertical RAM Address Start
        WriteCommand_Reg(0x0039,LCDD_DISP_Y-1-pActiveWin->brPX);
        //Window Vertical RAM Address End
        WriteCommand_Reg(0x0038,LCDD_DISP_Y-1-pActiveWin->tlPX);

        //Start point
        WriteCommand_Reg(0x0020,pActiveWin->tlPY);
        WriteCommand_Reg(0x0021,LCDD_DISP_Y-1-pActiveWin->tlPX);

    }
    else
    {
        //Window Horizontal RAM Address Start
        WriteCommand_Reg(0x0037,pActiveWin->tlPX);
        //Window Horizontal RAM Address End
        WriteCommand_Reg(0x0036,pActiveWin->brPX);
        //Window Vertical RAM Address Start
        WriteCommand_Reg(0x0039,pActiveWin->tlPY);
        //Window Vertical RAM Address End
        WriteCommand_Reg(0x0038,pActiveWin->brPY);

        //Start point
        WriteCommand_Reg(0x0020,pActiveWin->tlPX);
        WriteCommand_Reg(0x0021,pActiveWin->tlPY);
...
//    hal_HstSendEvent(0x88855516);
    ...
    else
    {
        ...
        if (frameBufferWin->fb.buffer != NULL)
        {
            ...
            //Window Horizontal RAM Address Start
            WriteCommand_Reg(0x0037,activeWin.tlPX);
            //Window Horizontal RAM Address End
            WriteCommand_Reg(0x0036,activeWin.brPX);
            //Window Vertical RAM Address Start
            WriteCommand_Reg(0x0039,activeWin.tlPY);
            //Window Vertical RAM Address End
            WriteCommand_Reg(0x0038,activeWin.brPY);

            //Start point
            WriteCommand_Reg(0x0020,activeWin.tlPX);
            WriteCommand_Reg(0x0021,activeWin.tlPY);
...
VOID lcddp_SetWindowsForSpiSensor()
{
    ...
    frameBufferWin.fb.width =  176;
    frameBufferWin.fb.height = 220;
    frameBufferWin.fb.colorFormat = LCDD_COLOR_FORMAT_RGB_565;

    frameBufferWin.roi.x=0;
    frameBufferWin.roi.y=0;
    frameBufferWin.roi.width= 176;
    frameBufferWin.roi.height= 220;
