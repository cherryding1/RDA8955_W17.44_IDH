                                  ...
                                  loff_t *ppos)
{
    ...
    for (i=0; i<iter; i++)
    {
        ...
        ret = priv->hw_host_to_card(priv, buf, len);
...
static void sdio_tput_test_read(struct rda5890_private *priv, int iter, int len)
{
    ...
    ret = priv->hw_host_to_card(priv, cmd, SDIO_TEST_CMD_LEN);
    if (ret)
    {
...
static void sdio_tput_test_write(struct rda5890_private *priv, int iter, int len)
{
    ...
    {
        if (!(i & 1))
            ret = priv->hw_host_to_card(priv, buf_1, len);
        else
            ret = priv->hw_host_to_card(priv, buf_2, len);
        if (ret)
        {
