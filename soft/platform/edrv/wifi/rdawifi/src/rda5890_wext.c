...
static struct iw_statistics *rda5890_get_wireless_stats(struct net_device *dev)
{
    ...
    priv->wstats.qual.level = priv->curbssparams.rssi;
                             ...
                             struct iw_point *dwrq, char *extra)
{
    ...
    if (priv->connect_status == MAC_CONNECTED)
    {
        memcpy(extra, priv->curbssparams.ssid,
               strlen(priv->curbssparams.ssid) + 1);
...
void rda5890_indicate_disconnected(struct rda5890_private *priv)
{
    ...
    RDA5890_DBGLAP(RDA5890_DA_WEXT, RDA5890_DL_VERB, "%s >>>\n", __func__);
                                ...
                                struct bss_descriptor *assoc_bss)
{
            ...
            && !priv->secinfo.WPA2enabled)
    {
        RDA5890_DBGLAP(RDA5890_DA_WEXT, RDA5890_DL_TRACE,
                       "%s, NO SEC\n", __func__);
        priv->imode = 0;
        rda5890_set_imode(priv, priv->imode);
    }
    else
    {
        u16 key_len = 0;
                ...
                && !priv->secinfo.WPA2enabled)
        {
            ...
            for (i=0; i<4; i++)
            {
                ...
                {
                    ret = rda5890_set_wepkey(priv, i,
                                             priv->wep_keys[i].key, priv->wep_keys[i].len);
                    if (ret)
                        goto out;
                }
            }
    ...
    {
        RDA5890_DBGLAP(RDA5890_DA_WEXT, RDA5890_DL_TRACE,
                       "%s, Open Auth, KEY_MGMT = %d\n", __func__, priv->secinfo.key_mgmt);
        if (priv->secinfo.key_mgmt == 0x01)
        {
            /* for 802.1x, set auth type to 0x04 */
            priv->authtype = 4;
            rda5890_set_authtype(priv, priv->authtype);
        }
        else
        {
            priv->authtype = 1;
...
void rda5890_assoc_worker(struct work_struct *work)
{
    ...
    ret = rda5890_set_ssid(priv, priv->assoc_ssid, priv->assoc_ssid_len);
    if (ret)
    {
    ...
    switch (mlme->cmd)
    {
        ...
        case IW_MLME_DISASSOC:
        {
            ...
            RDA5890_DBGLAP(RDA5890_DA_WEXT, RDA5890_DL_TRACE,
                           "DISASSOC\n");
            /* silently ignore */
            rda5890_set_ssid(dev->priv , ssid, 6);
        }
        break;
        default:
            RDA5890_DBGLAP(RDA5890_DA_WEXT, RDA5890_DL_TRACE,
