...
UINT8 tmp_read_data[8][1664] = {{0x00,},};

UINT8 tmp_read_data_flag =0x00;
...
//  hal_SdioOpen(0, wifi_IrqHandler);
    hal_SdioOpen(3, wifi_IrqHandler);
    ...
    tmp_addr = tmp_addr |(1<<28) |( 0x04 <<9)|0x37;
    ...
    MCD_TRACE(MCD_INFO_TRC, 0, "mcd_Open: Done");
    ...
    // g_SdioFreq = 20000000;
    g_SdioFreq = 10000000;   //modify 2011-10-20
...
//add flow contrl, 2011-12-26
    if(!wifi_context.firstWrite)
    {
        ...
        usingIrqSet();
        MCD_TRACE(MCD_INFO_TRC, 0, "new SDIO_WifiDataSingleBlockWrite: 0x%x,  0x%x",(ret & 0x08),count);

        while(((ret & 0x08)== 0x00)&&(count < RETRY_COUNT))
        {
            sxr_Sleep(100);
            tmp_addr = 0x00000000;
            tmp_addr = tmp_addr |(1<<28) |( 0x05 <<9);
...
//  MCD_TRACE(MCD_INFO_TRC, 0, "new SDIO_WifiDataSingleBlockWrite: 0x%x,  0x%x",(ret & 0x40),count);
        ...
        {
            tmp_addr = 0x88000000;
            tmp_addr = tmp_addr |(1<<28) |( 0x05 <<9)| 0x08;
            usingIrqClr();
...
//#if 0
        ...
        if(sidodata_size ==512)
            ...
            tmp_reg|=(sidodata_size|((UINT32)blockAddr)<<9);
   //     sxs_Dump(MCD_INFO_TRC, 0,(U8 *)pWrite, datalen);
        errStatus = mcd_SdioSendCmd(HAL_SDIO_CMD_IO_RW_EXTENDED, (UINT32)tmp_reg, cardResponse, FALSE,1,0);
        MCD_TRACE(MCD_INFO_TRC, 0, "CMD 53, arg: 0x%x, err status: %d,  response: 0x%x",tmp_reg,errStatus,cardResponse[3]);
   //     sxs_Dump(MCD_INFO_TRC, 0,(U8 *)cardResponse, 16);
...
//add flow contrl, 2011-12-26
    if(!wifi_context.firstWrite)
    {
        ...
        ret = read_test_cmd(52,tmp_addr);
        usingIrqSet();
        MCD_TRACE(MCD_INFO_TRC, 0, "new SDIO_WifiDataSingleBlockWriteMutiEx: 0x%x,  0x%x",(ret & 0x08),count);
...
{
    MCD_TRACE(MCD_INFO_TRC, 0, "SDIO_WifiDataSingleBlockWriteForMutiBlock, datalen: %d",datalen);
...
//add flow contrl, 2011-12-26
    if(!wifi_context.firstWrite)
    {
         ...
         UINT32 ret = 0x00;
            ...
            ret = read_test_cmd(52,tmp_addr);
            usingIrqSet();
            MCD_TRACE(MCD_INFO_TRC, 0, "new SDIO_WifiDataSingleBlockWriteForMutiBlock: 0x%x,  0x%x",(ret & 0x08),count);
...
#else
    MCD_TRACE(MCD_INFO_TRC, 0, "SDIO_WifiDataWrite:  wifi_sleep_flag: %d; ap_connected: %d;  ap_connecting: %d; preasso_timer_enable: %d\n",wifi_sleep_flag,ap_connected,ap_connecting,preasso_timer_enable);
...
INT8 wifi_getRcvBuf(VOID)
{
    ...
    UINT8 i =0;
    UINT8 tmp_flag = tmp_read_data_flag ;
    while(((tmp_flag&0x01) !=0x00)&&(i<8))
    {
        tmp_flag = tmp_flag>>1;
        i++;
    }
...
u8 fw_done = FALSE;
void process_frame(UINT8 bufIdx, UINT16 datalen)
{
    MCD_TRACE(MCD_INFO_TRC, 0, "process_frame bufIdx=%d,datalen=%d",bufIdx,datalen);
...
//#endif
    ...
    tmp_addr = 0x00000000;
    tmp_addr = tmp_addr |(0x01<<28) |( 0x05 <<9);
    ret = read_test_cmd(52,tmp_addr);

	//MCD_TRACE(MCD_INFO_TRC, 0, "Int pending:%x", ret);
	if (ret & 0x04) {
		tmp_addr = 0x80000000;
		tmp_addr = tmp_addr |(1<<28) |( 0x05 <<9)|0x04;
		send_test_cmd(52,tmp_addr);
        MCD_CS_EXIT;
        MCD_TRACE(MCD_INFO_TRC, 0, "test_Check_Sdio_DataIn,  ret & 0x04. ");
		return 0;

	} else if ((ret & 0x01) == 0) {
        MCD_CS_EXIT;
        MCD_TRACE(MCD_INFO_TRC, 0, "test_Check_Sdio_DataIn,  ret & 0x01. ");
		return 0;
	}
...
#else
            Mac_WifiFrameInd(bufIdx,datalen);
#endif
            //wifi_SdioSleep();
            // sxr_Sleep(200);
    ...
    UINT8 *wid_status, unsigned short wid_status_len)
{
    ...
    if (mac_status == MAC_CONNECTED)
    {
        ...
        if((0x00==wifi_context.dot11_mode) || (wifi_context.dot11_mode & (WEP|WEP_EXTENDED))) // no encryption
        {
			wifi_ConnectRsp(1);
            ip_WifiConnectedInd();
        }
...
static UINT32 wifi_ScanComplete(VOID)
{
    MCD_TRACE(MCD_INFO_TRC, 0, "wifi_ScanComplete\n");

    COS_EVENT ev;

    ev.nEventId = EV_CFW_WIFI_SCAN_COMPLETE;
    ...
    UINT8 *packet, unsigned short packet_len)
{
    ...
    //MCD_TRACE(MCD_INFO_TRC, 0, "rda5890_card_to_host >>>\n");
	  sxs_Dump(MCD_INFO_TRC, 0, packet, packet_len);
...
//#endif
        ...
        msg_type = packet[2];
        MCD_TRACE(MCD_INFO_TRC, 0, 
                         "Message Packet, startTime_ex: %d s,msg_type=%d\n", (startTime_ex/16384),msg_type );
        if(msg_type == 'I')
        {
            MCD_TRACE(MCD_INFO_TRC, 0, 
                             "Indication Message\n");
        ...
        {
        	u16 wid_id = packet[6] | packet[7]<<8;
			u8 sub_type = packet[11] & 0xFC;
			if (wid_id == WID_NETWORK_INFO) {
            MCD_TRACE(MCD_INFO_TRC, 0, 
                             "Network Message sub_type=%d\n",sub_type);
				if (sub_type == BEACON || sub_type == PROBE_RSP) {
#ifdef  __SCAN_INFO__
            rda5890_network_information(packet + 2,  rx_length - 2);
#endif
				}
                #ifdef WIFI_SNIFFER
                else if (sub_type == PROBE_REQ) {
                    rda_notify_probe_req_handler(packet + 21);
                }
                #endif
			}
            #ifdef WIFI_SMARTCONFIG
            else if (wid_id == WID_SMARTCFG_LDPCBCC_INFO)
            {
                u16 FrameLen = (u16) (packet[9] | (packet[10] << 8));
                u16 SNR = (u16) (packet[11] | (packet[12] << 8));
                s32 ret;
                if (FrameLen < 620) {
                    ret = rda_notify_smartlink_handler(FrameLen, NULL);
                    if (ret < 0)
                        MCD_TRACE(MCD_INFO_TRC, 0,"SmartConfig err=%d\n", ret);
                }
            }
            #endif
            else if (wid_id == WID_SCAN_CONNECT_RESULT) {
				u8 wid_value = packet[9];
				hal_HstSendEvent(0xaaaa0001);
				hal_HstSendEvent(wid_value);
				if (wid_value == 0x00) {
					MCD_TRACE(MCD_INFO_TRC, 0, 
								 "Scan complete event\n");
					wifi_ScanComplete();
				} else if (wid_value == 0x10) {
					if(ap_disconnecting)
					{
						re_associate_bss_Ex();
					}
    ...
    {
        //MCD_TRACE(MCD_INFO_TRC, 0, "Data Packet, len = %d\n", rx_length - 2);
...
#endif
int firmware_wid_request(UINT8 *wid_req, unsigned short wid_req_len)
{
    int ret = 0;
    unsigned short data_len;
     UINT8 * data_buf = wid_req  ;
    data_len = wid_req_len + 2;
    data_buf[0] = (UINT8)(data_len&0xFF);
    data_buf[1] = (UINT8)((data_len>>8)&0x0F);
    data_buf[1] |= 0x40;  // for Request(Q/W) 0x4
...
int rda5990_wid_request(UINT8 *wid_req, unsigned short wid_req_len)
{
    ...
    /* no_lock workaround */
    //kal_take_mutex(rda5890_wid_mutex);
/*   remove the wid_pending mechnism for 5991h
    if(rda5890_wid->wid_pending == 1)
    {
        UINT8 i =0;
...
int sdio_getInputData(void)
{
    ...
    */
    ...
    while(val_rsp==0x00)
    {
 //   	MCD_TRACE(MCD_INFO_TRC, 0, " to polling response of 52 cmd of tmp_addr %x \n. ");
        tmp_addr = 0x00000000;
//        tmp_addr = tmp_addr |(1<<28) |( 0x06 <<9);
        tmp_addr = tmp_addr |(1<<28) |( 0x05 <<9);
        ret = read_test_cmd(52,tmp_addr);
...
//       MCD_TRACE(MCD_INFO_TRC, 0, "tmp_read_data:");
        ...
        //        sxs_Dump(_MMI, 0, tmp_read_data, 80);
         int ret_read;
        ret_read = SDIO_WifiDataRead((U8*)0x08,tmp_read_data[bufIdx],datalen);
        hal_HstSendEvent(ret_read);
        if( 0== ret_read)
        {
            //rda5890_sendRxIntToDev();
            MCD_CS_EXIT;
...
int test_rda5890_generic_get_str(unsigned short wid, unsigned char *val, unsigned char *len)
{
    ...
    MCD_TRACE(MCD_INFO_TRC, 0, "test_rda5890_generic_get_str\n");
...
void show_bss_info_node(void)
{
    ...
    for(i=0; i<RDA5890_MAX_NETWORK_NUM; i++)
    {
        if(strlen(tmp[i].ssid)!=0)
        {
            ...
            showString((UINT8*)tmp[i].ssid);
            sxs_Dump(_MMI, 0, tmp[i].bssid, 6);
            num++;
...
*/
    ...
    wid_req[tmp_len] = (UINT8)(wid & 0x00FF);
    wid_req[tmp_len+1] = (UINT8)((wid & 0xFF00) >> 8);
    wid_req[tmp_len+2] = (UINT8)(ETH_ALEN);
    memcpy(wid_req +(tmp_len+ 3), wifi_context.cur_bssid, ETH_ALEN);
    tmp_len +=(3+ ETH_ALEN);
    ...
    wid_req[tmp_len+1] = (char)((wid&0xFF00) >> 8);
    wid_req[tmp_len+2] = 1;
    wid_req[tmp_len+3] = wifi_context.channel;
    tmp_len+=4;
    ...
    wid_req[tmp_len+1] = (char)((wid&0xFF00) >> 8);
    wid_req[tmp_len+2] = 1;
    wid_req[tmp_len+3] = SITE_SURVEY_OFF;
    tmp_len+=4;
...
int wifi_GetSSID(unsigned char *ssid)
{
    int ret=0x00;
    UINT8 widlen;
    ...
    MCD_TRACE(MCD_INFO_TRC, 0, "##########got  ssid: %s, widlen: %d\n", ssid, widlen);
    if (ret)
    {
        goto out;
    }
...
out:
    MCD_TRACE(MCD_INFO_TRC, 0, "wifi_GetSSID <<<, ret = %d\n", ret);
    return ret;
}
    ...
    unsigned short wid, unsigned short val)
{
    ...
    //MCD_TRACE(MCD_INFO_TRC, 0, "test_rda5890_generic_set_uchar ,wid_req: 0x%x,wid_req[0]=0x%x\n",wid_req, wid_req[0]);
    MCD_TRACE(MCD_INFO_TRC, 0, "test_rda5890_generic_set_ushort ,rda5890_wid->wid_msg_id: 0x%x,rda5890_wid->wid_pending: 0x%x\n",rda5890_wid->wid_msg_id,rda5890_wid->wid_pending);
...
int test_rda5890_set_wepkey(unsigned short index, unsigned char *key, unsigned char key_len)
{
    ...
    {
        for(i = 0; i < key_len; i++){
            if(key[i] >= 0x41 && key[i] <= 0x46)// switch 'A' to 'a'
                key[i] += 0x20;

            if(key[i] >= 0x61 && key[i] <= 0x66)//switch "ab" to 0xab
                key_str[i] = key[i] - 0x57;
            if(key[i] >= 0x30 && key[i] <= 0x39)
                key_str[i] = key[i] - 0x30;
            if(i%2 == 1)
                key_str[i/2] = key_str[i-1] << 4 | key_str[i];
    }

		key_str_len = key_len/2;
    }
    else
    {
	...
	wifi_context.WEP0[0] = 0;
	wifi_context.WEP0[1] = key_str_len;
    // stop_wifiPollTimer();
        ret = test_rda5890_generic_set_str(WID_ADD_WEP_KEY, wifi_context.WEP0, key_str_len+2);
        if (ret) {
...
int test_start_associate_bss_Ex()
{
	int ret;
...
//  memcpy(&(bss_desc_Used.data), tmp_ssid, sizeof(RDA5890_BSS_DESC));
	...
	if (strlen(wifi_context.cur_pwd)) {
		initWifiWapSm();
	}

	ret = wpa_supplicant_set_suites(bss_desc_Used.wpa_ie, bss_desc_Used.wpa_ie_len, bss_desc_Used.rsn_ie, bss_desc_Used.rsn_ie_len);
	MCD_TRACE(MCD_INFO_TRC, 0,  "wpa_supplicant_set_suites ret:%d. 11i_mode:%x, auth_type:%x\n",
		ret, wifi_context.dot11_mode, wifi_context.auth_type);
    if(wifi_context.dot11_mode)//encryption enabale
    {

        //stop_wifiPollTimer();

        if(wifi_context.dot11_mode & (WEP|WEP_EXTENDED))//for WEP
        {
			...
			if (wifi_context.auth_type == SHARED_KEY)
...
//
        ...
        else
        {
            ...
            setWpaAddr(wifi_context.cur_bssid, localMac, wifi_context.cur_ssid);
            setWifiSmField(wifi_context.cur_pwd);
...
int re_associate_bss_Ex(VOID)
{
	...
	if (associate_tries > 5)
		wifi_ConnectRsp(0);
	else
    	return test_rda5890_set_ssid_Ex(wifi_context.cur_ssid, strlen(wifi_context.cur_ssid));
}
...
//    MCD_TRACE(MCD_INFO_TRC, 0, "########## after wifi_init, chip id: 0x%x;  dma enable: %d\n",hal_SdioGetChipId(),g_SdioIsDmaEnable);
	...
	sxs_Dump(MCD_INFO_TRC, 0, localMac, 6);
    }

    fw_done = FALSE;
    wifi_FirmwareDownload();
...
VOID wifi_SScanApsEx_1(u8 channel, u16 time)
{
	wifi_start_sscan_set(channel, time);
}
...
void store_ConnectInfo(wifi_conn_t* conn)
{
    ...
    memset(wifi_context.cur_bssid,0x00,6);
    memset(wifi_context.cur_pwd,0x00,64);
    memset(wifi_context.cur_ssid,0x00,33);
    memset(wifi_context.WEP0,0x00,32);

    strcpy(wifi_context.cur_ssid, conn->ssid);

	if (conn->password)
    	strcpy(wifi_context.cur_pwd, conn->password);

	if (conn->pwd_len)
		wifi_context.dot11_mode = ENCRYPT_ENABLED;
	else
		wifi_context.dot11_mode = NO_ENCRYPT;
    memset(&bss_desc_Used,0,sizeof(struct bss_descriptor ));
	bss_desc_Used.data.rssi = -128;
	for(i=0; i<RDA5890_MAX_NETWORK_NUM; i++)
...

{
    ...
    //sxr_Sleep(500*16384/1000);
		...
		if (strcmp(tmp[i].ssid, conn->ssid) == 0) {
			if (tmp[i].rssi > bss_desc_Used.data.rssi) {
				bss_desc_Used.data.rssi = tmp[i].rssi;
				memcpy(bss_desc_Used.data.bssid, tmp[i].bssid, ETH_ALEN);
				strcpy(bss_desc_Used.data.ssid, tmp[i].ssid);

				bss_desc_Used.wpa_ie = tmp[i].wpa_ie;
				bss_desc_Used.wpa_ie_len = tmp[i].wpa_ie_len;
				bss_desc_Used.rsn_ie = tmp[i].rsn_ie;
				bss_desc_Used.rsn_ie_len = tmp[i].rsn_ie_len;
				bss_desc_Used.data.channel = tmp[i].channel;
				wifi_context.channel = tmp[i].channel;
				memcpy(wifi_context.cur_bssid, tmp[i].bssid, ETH_ALEN);
			}
    ...
    MCD_TRACE(MCD_INFO_TRC, 0, "ssid, pwd, channel:%d\n", wifi_context.channel);
...
VOID wifi_ConnectApEx_wifi(UINT16 dur)
{
    ...
    //test_rda5890_set_preasso_sleep(0x01000020);//enable wifi sleep pre-association set preasso time to (2^5)/10 s
    test_start_associate_bss_Ex();

    //hal_sdioDataInIrqMaskSet();
    start_connect_ap_timer(dur);
    //useingIrq = TRUE;
}
#endif
...
//sxr_Sleep(500*16384/1000);
    ...
    preasso_timer_enable = TRUE;
    //sxr_Sleep (300);
    test_rda5890_generic_set_ulong(WID_DISCONNECT_REASON,WLAN_REASON_DEAUTH_LEAVING);
    if(-1== sdio_getInputData())
    {
        MCD_TRACE(MCD_INFO_TRC, 0, "WID_DISCONNECT_REASON sdio_getInputData fail\n");
    }
    test_rda5890_generic_set_uchar(WID_DISCONNECT,0x00);
    if(-1== sdio_getInputData())
    {
        MCD_TRACE(MCD_INFO_TRC, 0, "WID_DISCONNECT sdio_getInputData fail\n");
    }
    //sxr_Sleep (300);
