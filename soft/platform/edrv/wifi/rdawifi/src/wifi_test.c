...
typedef void (*UART_RX_CB_FUNC)(uint8 *buff,uint32 len) ;
typedef enum
{
    uart_port1=0,
    uart_port2,
...
extern void rda5868_uart_stop(void);

extern void rda5868_uart_start(void);
extern void rda5868_uart_configure(unsigned baud,char flow_control);
extern unsigned int rda5868_uart_rx(void);
extern unsigned char rda5868_uart_tx(const uint8 *buf, unsigned short num_to_send, unsigned short *num_send);
extern void rdabt_uart_eint_hdlr(void);
extern void rdabt_uart_register_eint(void);
extern void RDA5868_uart_rx_cb(UART_PORT port);
extern void rdabt_set_uart_rx_cb(UART_RX_CB_FUNC func);
extern void RDA5868_uart_register_cb(void);

extern void UART_Register_RX_cb(PORT port,UINT16 owner,void(*callfunc)(UINT8 port));
...
void wifi_test_init(void )
{
    ...
    rdabt_set_uart_rx_cb(wifi_handle_received_data_cb);
...
void wifi_test_tx(const uint8 *buf, unsigned short num_to_send, unsigned short *num_send)
{
    ...
    ret = rda5868_uart_tx(buf, num_to_send, num_send);
}
