...
extern VOID AT_Result_OK(UINT32 uReturnValue, UINT32 uResultCode, UINT8  nDelayTime, UINT8* pBuffer, UINT16 nDataSize, UINT8 nDLCI, UINT8 nSim);
...
T_WifiOpState g_tWifiOperating;
...
const wifi_cxt_t *WifiCxtConfGet(void)
{
    return &wifi_context;
}
...
PUBLIC MCD_ERR_T SDIO_mcd_Open(MCD_CSD_T* mcdCsd)
{
    ...
    */
    ...
    // g_SdioFreq = 20000000;
    g_SdioFreq = 8000000;   //modify 2011-10-20
...
U8 SDIO_WifiDataSingleBlockWrite(UINT32 blockAddr, const U8* pWrite, UINT32 datalen)
{
...

    {
        ...
        while(! hal_SdioTransferFiFoDone())
        {
            ...
            {
                MCD_TRACE(MCD_INFO_TRC, 0, "write on Sdmmc cmd timeout");
                //
                //          Kill_pollTimer();
    ...
    if(g_SdioIsDmaEnable)
    {
        ...
        while(!hal_SdioTransferDone())
        {
            ...
            {
                MCD_TRACE(MCD_INFO_TRC, 0, "Write on Sdmmc dma timeout");
                // Abort the transfert.
                hal_SdioStopTransfer(&g_SdioTransfer);
                return MCD_ERR_CARD_TIMEOUT;
            }
    ...
    else
    {
        ...
        while(! hal_SdioTransferFiFoDone())
        {
            ...
            if (hal_TimGetUpTime() - startTime >  (MCD_READ_TIMEOUT*blockNum*5))
            {
                MCD_TRACE(MCD_INFO_TRC, 0, "write on Sdmmc fifo timeout");
                //
                //          Kill_pollTimer();
                usingIrqSet();
                wifi_ResetReq();
...
U8 SDIO_WifiDataSingleBlockWriteMutiEx(UINT32 blockAddr, const U8* pWrite, UINT32 datalen)
{
    ...
    else
    {
        ...
        if(sidodata_size ==512)
            ...
            tmp_reg|=(sidodata_size|((UINT32)blockAddr)<<9);
        errStatus = mcd_SdioSendCmd(53, (UINT32)tmp_reg, cardResponse, FALSE,1,0);
        MCD_TRACE(MCD_INFO_TRC, 0, "CMD 53, arg: 0x%x, err status: %d,  response: 0x%x",tmp_reg,errStatus,cardResponse[3]);
        sxs_Dump(MCD_INFO_TRC, 0,(U8 *)cardResponse, 16);
        //sxr_Sleep(10);
...
U8 SDIO_WifiDataMultBlockWrite(UINT32 blockAddr, const U8* pWrite, UINT32 datalen)
{
    ...
    errStatus = mcd_SdioSendCmd(53, (UINT32)tmp_reg, cardResponse, FALSE,1,1);
    MCD_TRACE(MCD_INFO_TRC, 0, "CMD 53, arg: 0x%x, err status: %d,  response: 0x%x",tmp_reg,errStatus,cardResponse[3]);
    sxs_Dump(MCD_INFO_TRC, 0,(U8 *)cardResponse, 16);
...
U8 SDIO_WifiDataSingleBlockRead(U8* blockAddr, U8* pRead, UINT32 datalen)
{
    ...
    if((wifi_version_flag== WIFI_VERSION_E) ||(wifi_version_flag== WIFI_VERSION_F))
    {
        ...
        //  tmp_reg|=(sidodata_size|((UINT32)blockAddr)<<9);
        //tmp_reg|=blockAddr;
        //   errStatus = mcd_SdioSendCmd(53, (UINT32)tmp_reg, cardResponse, FALSE);
        MCD_TRACE(MCD_INFO_TRC, 0, "CMD 53, arg: 0x%x, err status: %d,  response: 0x%x",tmp_reg,errStatus,cardResponse[3]);
        sxs_Dump(MCD_INFO_TRC, 0,(U8 *)cardResponse, 16);
        U8 iii =0;
        UINT32 ret_val =0;
...
// MCD_TRACE(MCD_INFO_TRC, 0, "read FIFO 5: 0x%x",hal_read_databyFIFO());
    ...
    if(g_SdioIsDmaEnable)
    {
        ...
        while(!hal_SdioTransferDone())
        {
            ...
            {
                MCD_TRACE(MCD_INFO_TRC, 0, "Read on Sdio timeout  -- 1");
                //  Kill_pollTimer();
    ...
    else
    {
        ...
        while(! hal_SdioTransferFiFoDone())
        {
            ...
            if (hal_TimGetUpTime() - startTime >  (MCD_READ_TIMEOUT*blockNum*3))
            {
                MCD_TRACE(MCD_INFO_TRC, 0, "Read on Sdio timeout  -- 2");
                wifi_ResetReq();
                // Kill_pollTimer();
                return MCD_ERR_CARD_TIMEOUT;
...
int test_rda5890_start_scan(void)
{
    ...
    //hal_sdioDataInIrqMaskClear();
    MCD_TRACE(MCD_INFO_TRC, 0,   "rda5890_start_scan begin>>>\n");
    ...
    {
        MCD_TRACE(MCD_INFO_TRC, 0,"rda5890_wid_request fail, ret = %d\n", ret);
        MCD_TRACE(MCD_INFO_TRC, 0, "rda5890_wid_request: fail\r\n", 0);
...
int test_rda5890_set_ssid_Ex(unsigned char *ssid, unsigned char ssid_len)
{
    ...
    UINT8* wid_req_p;
    UINT8 wid_req[RDA5890_MAX_WID_LEN];
    //unsigned short tmp_len = 7 + ssid_len;
    //  UINT8 wid_rsp[RDA5890_MAX_WID_LEN];
    //  unsigned short wid_rsp_len = RDA5890_MAX_WID_LEN;
    UINT8 wid_msg_id = rda5890_wid->wid_msg_id;
...
//   wid_req[3] = (priv->imode == 0x05) ? 0x07 : priv->imode;  //for wep104 need set imode 0x07 firmware problem
...

    {
        ...
        wid_req[tmp_len+2] = 1;
        wid_req[tmp_len+3] = (wifi_context.dot11_mode == 0x05)? 0x07:wifi_context.dot11_mode;
        tmp_len+=4;
    }
    ...
    wid = WID_NETWORK_INFO_EN;
    wid_req[tmp_len+0] = (char)(wid & 0x00FF);
    wid_req[tmp_len+1] = (char)((wid & 0xFF00) >> 8);
    wid_req[tmp_len+2] = 1;
    wid_req[tmp_len+3] = 0;
    tmp_len += 4;
...
//  wid_req += 4;
    ...
    //if ((rda_wlan_version() == WIFI_VERSION_D) || (rda_wlan_version() == WIFI_VERSION_E)  || (rda_wlan_version() == WIFI_VERSION_5991E))
    {

        wid_req_p = wid_req + tmp_len;
        wid = WID_MEMORY_ADDRESS;
        wid_req_p[0] = (char)(wid&0x00FF);
        wid_req_p[1] = (char)((wid&0xFF00) >> 8);
        wid_req_p[2] = 4;
        wid_req_p[3] = 0xec;
        wid_req_p[4] = 0x81;
        wid_req_p[5] = 0x10;
        ...
        wid_req_p[1] = (char)((wid&0xFF00) >> 8);
        wid_req_p[2] = 4;
        wid_req_p[3] = wifi_context.cur_bssid[0];
        wid_req_p[4] = wifi_context.cur_bssid[1];
        wid_req_p[5] = wifi_context.cur_bssid[2];
        wid_req_p[6] = wifi_context.cur_bssid[3];
        tmp_len += 7;
        wid_req_p += 7;
        ...
        wid_req_p[1] = (char)((wid&0xFF00) >> 8);
        wid_req_p[2] = 4;
        wid_req_p[3] = wifi_context.cur_bssid[4];
        wid_req_p[4] = wifi_context.cur_bssid[5];
        wid_req_p[5] = 0;
        wid_req_p[6] = 0;
        tmp_len += 7;
        wid_req_p += 7;
    ...
    wid_req_p[1] = (char)((wid & 0xFF00) >> 8);
    wid_req_p[2] = 6;
    memcpy(&wid_req_p[3], wifi_context.cur_bssid, 6);
    tmp_len += 9;
    wid_req_p += 9;
    ...
    wid_req_p[0] = (char)(wid & 0x00FF);
    wid_req_p[1] = (char)((wid & 0xFF00) >> 8);
    wid_req_p[2] = ssid_len;
    memcpy(wid_req_p + 3, ssid, ssid_len);
    tmp_len += 3 + ssid_len;
    wid_req_p += 3 + ssid_len;
    ...
    else
    {
        if(wifi_version_flag== WIFI_VERSION_D )
        {
            ...
            len_tmp = len_tmp + count*14;
                ...
                ||(wifi_version_flag== WIFI_VERSION_F)
                ||(wifi_version_flag== WIFI_VERSION_5991)||(wifi_version_flag== WIFI_VERSION_5991E))
        {
            len_tmp = len-2;
...
VOID wifi_setMac(UINT8 *mac_addr, UINT8 len)
{
    ...
    if (ret)
    {
        ...
        MCD_TRACE(MCD_INFO_TRC, 0, "wifi_setMac: 001\r\n", 0);
        return ;
    }
    ...
    if(-1== sdio_getInputData())
    {
        ...
        restart_wifiPollTimer();
        //hal_sdioDataInIrqMaskSet();
        MCD_TRACE(MCD_INFO_TRC, 0, "wifi_setMac: 002\r\n", 0);
        return -1;
    }
}
/*
...
const struct bss_descriptor *WifiBssDescriporGet(void)
{
    return &bss_desc_Used;
}
...
INT  test_rda5890_set_preasso_sleep( UINT32 val)
{
    ...
    if(-1== sdio_getInputData())
    {
        restart_wifiPollTimer();
        //hal_sdioDataInIrqMaskSet();
        MCD_TRACE(MCD_INFO_TRC, 0,"test_rda5890_set_preasso_sleep: -1\r\n",0);
...
INT  test_rda5890_set_pm_mode(UINT8 val)
{
    ...
    if(-1== sdio_getInputData())
    {
        restart_wifiPollTimer();
        //hal_sdioDataInIrqMaskSet();
        MCD_TRACE(MCD_INFO_TRC, 0, "test_rda5890_set_pm_mode: -1\r\n", 0);
...
INT  test_rda5890_disable_self_cts(void)
{
    ...
    {
        restart_wifiPollTimer();
        MCD_TRACE(MCD_INFO_TRC, 0, "test_rda5890_disable_self_cts: -1\r\n", 0);
        return -1;
    }
    return 0;
}
...
INT  test_rda5890_disable_block_bt(void)
{
    ...
    {
        restart_wifiPollTimer();
        MCD_TRACE(MCD_INFO_TRC, 0, "test_rda5890_disable_block_bt: -1\r\n", 0);
        return -1;
    }
    return 0;
}
...
void  test_rda5890_set_active_scan_time(void)
{
    ...
    {
        restart_wifiPollTimer();
        MCD_TRACE(MCD_INFO_TRC, 0, "test_rda5890_set_active_scan_time: -1\r\n", 0);
        return;
    }
    return ;
}
...
int test_rda5890_set_link_loss_threshold(UINT8 threshold)
{
    ...
    {
        restart_wifiPollTimer();
        MCD_TRACE(MCD_INFO_TRC, 0, "test_rda5890_set_link_loss_threshold: -1\r\n", 0);
        return -1;
    }
    return 0;
}
...
UINT8 tmpdata[1025];
extern int rda5991_sdio_patch(void);
extern int rda5990_sdio_patch(void);
...
VOID wifi_PowerOnEx_1(VOID)
{
    ...
    if(SDIO_mcd_Open(&mmc_csd)==MCD_ERR_NO_CARD)
    {
        ...
        wifi_PowerOnFail();
        //          MCD_ASSERT(NULL, "IsSdioBusy");
        //wifi_ResetReqEx();
        MCD_TRACE(MCD_INFO_TRC, 0, "SDIO_mcd_Open: Error - no card\r\n", 0);
        return;
    }
    ...
    {
        MCD_TRACE(MCD_INFO_TRC, 0,  "Ssdio_getInputData, -1\n");
        MCD_TRACE(MCD_INFO_TRC, 0, "sdio_getInputData: have problem?\r\n", 0);
    ...
    switch (chipVer)
    {
        case WIFI_VERSION_5991:
            MCD_TRACE(MCD_INFO_TRC, 0,"##########  rda5991_sdio_patch begin\n");

            rda5991_sdio_patch();

            MCD_TRACE(MCD_INFO_TRC, 0,"##########  rda5991_sdio_patch end\n");
            break;
        case WIFI_VERSION_5991E:

            MCD_TRACE(MCD_INFO_TRC, 0,"##########  rda5991e_sdio_patch begin\n");
            rda5991e_sdio_patch();

            MCD_TRACE(MCD_INFO_TRC, 0,"##########  rda5991e_sdio_patch end\n");
            break;
        case WIFI_VERSION_D:
        case WIFI_VERSION_E:
        case WIFI_VERSION_F:
            ...
            break;
        default:
            MCD_TRACE(MCD_INFO_TRC, 0, "unkown version\r\n", 0);
...
#error
    ...
    MCD_TRACE(MCD_INFO_TRC, 0, "#########> SDIO end!!\r\n", 0);
}
...
//wifi_init_patch_dataEx();
    ...
    rda5890_WifWakeUp();
    test_rda5890_set_preasso_sleep(0);
    //test_rda5890_set_preasso_sleep(0x01000020);//enable wifi sleep pre-association set preasso time to (2^5)/10 s
    //0x00800080
    test_rda5890_set_pm_mode(0);// //add 2012 06 14  disable wf sleep
    test_rda5890_disable_self_cts();
    test_rda5890_set_active_scan_time();
    test_rda5890_disable_block_bt();
    test_rda5890_set_link_loss_threshold(0x50);
...
void store_ConnectInfo(RDA5890_BSS_DESC* tmp_bss, UINT8 len, wifi_conn_t* conn)
{
    ...
    wifi_password = wifi_context.cur_pwd;
    MCD_TRACE(MCD_INFO_TRC, 0,"ssid=%s,pwd:%s\n", wifi_context.cur_ssid, wifi_context.cur_pwd);
...
VOID wifi_DisconnectAp_wifi(void)
{
    ...
    /* hal_DbgAssert("%s(%s,%d)", __func__,__FILE__,__LINE__); */
    preasso_timer_enable = TRUE;
    //sxr_Sleep (300);
...
UINT32 wifi_ScanRsp(VOID)
{
    COS_EVENT ev;
    UINT8 i =0;
    RDA5890_BSS_DESC *ptBss = NULL;
#if 0

    MCD_TRACE(MCD_INFO_TRC, 0,"wifi_ScanRsp\n");

    MCD_TRACE(MCD_INFO_TRC, 0,  "bss_Rst.count=%d \n", bss_Rst.count);
    if(bss_Rst.count >0)
    {
        RDA5890_BSS_DESC* tmpbss =( (RDA5890_BSS_DESC *)(bss_Rst.bss_descs));
        for(i =0; i < bss_Rst.count; i++)
        {
            ...
            MCD_TRACE(MCD_INFO_TRC, 0,"ssid=%s\n",ptBss->ssid);
            MCD_TRACE(MCD_INFO_TRC, 0,"bss_type=%d\n",ptBss->bss_type);
            MCD_TRACE(MCD_INFO_TRC, 0,"channel=%d\n",ptBss->channel);
            MCD_TRACE(MCD_INFO_TRC, 0,"dot11i_info=0x%02x\n",ptBss->dot11i_info);
            MCD_TRACE(MCD_INFO_TRC, 0,"bssid=%02x:%02x:%02x:%02x:%02x:%02x\n",\
                      ptBss->bssid[0],ptBss->bssid[1],ptBss->bssid[2],ptBss->bssid[3],ptBss->bssid[4],ptBss->bssid[5]);
            MCD_TRACE(MCD_INFO_TRC, 0,"rssi=%d\n",ptBss->rssi);
            MCD_TRACE(MCD_INFO_TRC, 0,"auth_info=%02x\n",ptBss->auth_info);
            MCD_TRACE(MCD_INFO_TRC, 0,"rsn_cap[]=%02x %02x\n",ptBss->rsn_cap[0],ptBss->rsn_cap[1]);
            MCD_TRACE(MCD_INFO_TRC, 0,"rsn_grp=%02x\n",ptBss->rsn_grp);
            MCD_TRACE(MCD_INFO_TRC, 0,"rsn_pairwise[]=%02x-%02x-%02x\n",ptBss->rsn_pairwise[0],ptBss->rsn_pairwise[1],ptBss->rsn_pairwise[2]);
            MCD_TRACE(MCD_INFO_TRC, 0,"rsn_auth[]=%02x-%02x-%02x\n",ptBss->rsn_auth[0],ptBss->rsn_auth[1],ptBss->rsn_auth[2]);
            MCD_TRACE(MCD_INFO_TRC, 0,"wpa_grp=%02x\n",ptBss->wpa_grp);
            MCD_TRACE(MCD_INFO_TRC, 0,"wpa_pairwise[]=%02x-%02x-%02x\n",ptBss->wpa_pairwise[0],ptBss->wpa_pairwise[1],ptBss->wpa_pairwise[2]);
            MCD_TRACE(MCD_INFO_TRC, 0,"wpa_auth[]=%02x-%02x-%02x\n",ptBss->wpa_auth[0],ptBss->wpa_auth[1],ptBss->wpa_auth[2]);
        }
    }
    ...
    g_tWifiOperating.bWifiOpScan= FALSE;
...
UINT32 wifi_ConnectRsp(UINT8 flag)
{
    ...
    g_tWifiOperating.bWifiOpConn= FALSE;
    return 0;
}
...
UINT32 wifi_PowerOnRsp(void)
{
    ...
    g_tWifiOperating.bWifiOpInit = FALSE;
    g_tWifiOperating.bWifiInited = TRUE;
    return 0;
}
...
UINT32 wifi_PowerOnFail(void)
{
    ...
    COS_SendEvent(COS_GetDefaultMmiTaskHandle(DEFAULT_MMI_TASK_ID), &ev, COS_WAIT_FOREVER, COS_EVENT_PRI_NORMAL);
    g_tWifiOperating.bWifiOpInit = FALSE;
    g_tWifiOperating.bWifiInited = FALSE;
    return 0;
}
UINT32 wifi_PowerOffRsp(void)
{
    ...
    ev.nParam3  = 0;
    COS_SendEvent(COS_GetDefaultMmiTaskHandle(DEFAULT_MMI_TASK_ID), &ev, COS_WAIT_FOREVER, COS_EVENT_PRI_NORMAL);
    g_tWifiOperating.bWifiOpPwroff = FALSE;
    g_tWifiOperating.bWifiInited = FALSE;
    return 0;
}
///////////////////////////////////////////////////////
...
INT8 sdio_getWidResp(void)
{
    ...
    while(rda5890_wid->wid_pending == 1)
    {
        ...
        while(val_rsp==0x00)
        {
            ...
            {
                MCD_CS_EXIT;
                MCD_TRACE(MCD_INFO_TRC, 0, "sdio_getWidResp return");
                return -1;
            }

            //if (hal_TimGetUpTime() - startTime >  (MCD_READ_TIMEOUT*5))
            {
                //MCD_TRACE(MCD_INFO_TRC, 0, "sdio_getWidResp timeout");
            }
...
int wifi_sdio_patch_core_32(const UINT32 array[][2], UINT32 n_nelements)
{
    MCD_TRACE(MCD_INFO_TRC, 0,"write_wifi_Core_dataEx, wifi_version: 0x%x",wifi_version_flag);
    ...
    UINT16 tmp_len ;
    char wid_msg_id = rda5890_wid->wid_msg_id;
    rda5890_wid->wid = WID_MEMORY_ADDRESS;
    rda5890_wid->wid_rsp_len =RDA5890_MAX_WID_LEN;
    memset(rda5890_wid->wid_rsp,0x0,512);
    ...
    while(patch_dada_count < patch_len)
    {
        ...
        {
            // write address
            wid_req[tmp_len] = (char)(WID_MEMORY_ADDRESS&0x00FF);
            wid_req[tmp_len+1] = (char)((WID_MEMORY_ADDRESS&0xFF00) >> 8);
            ...
            tmp_len= tmp_len + 3+ sizeof(UINT32);
            //write data
            wid_req[tmp_len] = (char)(WID_MEMORY_ACCESS_32BIT&0x00FF);
            wid_req[tmp_len+1] = (char)((WID_MEMORY_ACCESS_32BIT&0xFF00) >> 8);
    ...
    if (ret)
    {
        MCD_TRACE(MCD_INFO_TRC, 0,"rda5991_sdio_patch_core_32 fail, ret = %d\n", ret);
...
INT8 wifi_sdio_byte_write(const u8 arr[][2], UINT32 n_nelements)
{
    ...
    char wid_msg_id = rda5890_wid->wid_msg_id;
    rda5890_wid->wid = WID_PHY_ACTIVE_REG;
    rda5890_wid->wid_rsp_len =RDA5890_MAX_WID_LEN;
    memset(rda5890_wid->wid_rsp,0x0, 512);
    ...
    while(patch_dada_count < patch_len)
    {
        ...
        {
            // write address
            wid_req[tmp_len] = (char)(WID_PHY_ACTIVE_REG&0x00FF);
            wid_req[tmp_len+1] = (char)((WID_PHY_ACTIVE_REG&0xFF00) >> 8);
            ...
            //write data
            wid_req[tmp_len] = (char)(WID_PHY_ACTIVE_REG_VAL&0x00FF);
            wid_req[tmp_len+1] = (char)((WID_PHY_ACTIVE_REG_VAL&0xFF00) >> 8);
    ...
    if (ret)
    {
        MCD_TRACE(MCD_INFO_TRC, 0,"rda5991_sdio_byte_write fail, ret = %d\n", ret);
...
T_WifiOpState *WifiOpSateGet(void)
{
    return &g_tWifiOperating;
}
