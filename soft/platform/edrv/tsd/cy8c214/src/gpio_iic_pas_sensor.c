...
BOOL tsd_cap_gpio_i2c_init_sensor(VOID)
{
    if (init_first == FALSE)
    {
        init_first = TRUE;
...
//(0x07,0x11);
// data_sensor(0x0a,&Addr,1,&Data,1)
BOOL tsd_cap_gpio_i2c_write_data_sensor(UINT32 addr,UINT8 * regaddr,UINT8 addrlen,UINT8 *data, UINT8 datalen)
{
    ...
    if(1==gpio_i2c_TxByte(addr<<1))
    {
        hal_SysExitCriticalSection(scStatus);
        hal_HstSendEvent(0x99660001);
        return FALSE;
    }//chip adress

    for(i=addrlen; i>0; i--,regaddr++) //addr
    {
        if(gpio_i2c_TxByte(*regaddr) == 1)
        {
            hal_SysExitCriticalSection(scStatus);
            hal_HstSendEvent(0x99660002);
            return FALSE;
        }
    }
    ...
    {
        ack_no = TRUE;
        if(gpio_i2c_TxByte(*data) == 1)
        {
            hal_SysExitCriticalSection(scStatus);
            hal_HstSendEvent(0x99660003);
            return FALSE;
        }
    }
    gpio_i2c_Stop();
...
UINT8 tsd_cap_gpio_i2c_read_data_sensor_continue(UINT32 addr,UINT8 *datas, UINT8 datalen)
{
    ...
    gpio_i2c_Start();//start
    if(1== gpio_i2c_TxByte((addr<<1)+1)) hal_DbgAssert("no ack here 1");
