...
typedef struct
{
    ...
    BOOL                   present;
    BOOL                   capcity_small;
    BOOL                   capcity_small_special;// for some error
    MCD_CARD_PRO_T           try_pro;

} MCD_TWO_SDMMC_CONTROL;

#endif

MCD_TWO_SDMMC_CONTROL g_sdmmc_info1 =
{
    .open = FALSE,
    .oper_fail = FALSE,
    .init = FALSE,
    .present = FALSE,
    .capcity_small = FALSE,
    .capcity_small_special = FALSE,
    .try_pro = MCD_CARD_NORMAL,

};

extern MCD_TWO_SDMMC_CONTROL g_sdmmc_info2;
...
//  a bad CRC.
// =============================================================================
extern HAL_SDMMC_OP_STATUS_T hal_SdmmcGetOpStatus(VOID);
...
#endif
    MCD_TRACE(TSTDOUT, 0, "CSD1:block number     = %d", csd->blockNumber        );

    // block number = 63904  (mark 128M, actually is 28M )
    return MCD_ERR_NO;
}
...
PUBLIC MCD_ERR_T mcd_sdmmc_Open_for_two_sdmmc_1(MCD_CSD_T* mcdCsd, MCD_CARD_VER mcdVer,MCD_SDMMC_SPI_IF* mcdConfig)
{
    MCD_ERR_T                  errStatus   = MCD_ERR_NO;
    ...
    hal_HstSendEvent(0xaa65432b);
    MCD_TRACE(TSTDOUT, 0, "mcd_sdmmc_Open starts ...");
    ...
    if(g_mcdConfig->cardDetectGpio != HAL_GPIO_NONE)
    {
        ...
        {
            MCD_TRACE(TSTDOUT, 0, "mcd_sdmmc_Open: GPIO detection - no card");
            MCD_CS_EXIT;
    ...
    if (g_mcdStatus == MCD_STATUS_OPEN)
    {
        ...
        MCD_TRACE(TSTDOUT, 0, "mcd_sdmmc_Open: Already opened");
    ...
    if(g_sdmmc_info1.try_pro != MCD_CARD_NORMAL )            //if(MCD_CARD_V1 == mcdVer)
    {
        hal_SdmmcOpen(0x05);
    }
    else
    ...
    else
    {
        ...
        if(g_sdmmc_info1.try_pro == MCD_CARD_NORMAL )
        {
    ...
    else if( g_sdmmc_info1.try_pro == MCD_CARD_SMALL)
    {
        g_mcdSdmmcFreq = 6000000;
    }
    else if(g_sdmmc_info1.try_pro == MCD_CARD_SMALL_SPECIAL)
    {
...
#endif
PUBLIC MCD_ERR_T mcd_Init_sdmmc1(MCD_CSD_T* mcdCsd, MCD_CARD_VER mcdVer,MCD_SDMMC_SPI_IF* mcdConfig)
{
    MCD_ERR_T  errStatus1;
    MCD_TRACE(TSTDOUT, 0, "mcd1: init ");
    mcd_sdmmc_rst_volt();
    errStatus1 = mcd_sdmmc_Open_for_two_sdmmc_1(&(g_sdmmc_info1.csdinfo), mcdVer,mcdConfig);
    if (MCD_ERR_NO == errStatus1)
    {
        g_sdmmc_info1.present = TRUE;
        g_sdmmc_info1.init = TRUE;
        MCD_TRACE(TSTDOUT, 0, "mcd1: init ok 1 ");
        return MCD_ERR_NO;

    }
    else
    {
        mcd_sdmmc_rst_volt(); MCD_TRACE(TSTDOUT, 0, "mcd1: init fail 1 ");
        g_sdmmc_info1.try_pro = MCD_CARD_SMALL;
        errStatus1 = mcd_sdmmc_Open_for_two_sdmmc_1(&(g_sdmmc_info1.csdinfo), mcdVer,mcdConfig);
        if (MCD_ERR_NO == errStatus1)
        {
            g_sdmmc_info1.present = TRUE;
            g_sdmmc_info1.init = TRUE;
            MCD_TRACE(TSTDOUT, 0, "mcd1: init ok 2");
            return MCD_ERR_NO;
        }
        else
        {
            mcd_sdmmc_rst_volt(); MCD_TRACE(TSTDOUT, 0, "mcd1: init fail 2 ");
            g_sdmmc_info1.try_pro = MCD_CARD_SMALL_SPECIAL;
            errStatus1 = mcd_sdmmc_Open_for_two_sdmmc_1(&(g_sdmmc_info1.csdinfo), mcdVer,mcdConfig);
            if (MCD_ERR_NO == errStatus1)
            {
                g_sdmmc_info1.present = TRUE;
                g_sdmmc_info1.init = TRUE;
                MCD_TRACE(TSTDOUT, 0, "mcd1: init ok 3");
                return MCD_ERR_NO;
            }
            else
            {
                MCD_TRACE(TSTDOUT, 0, "mcd1: no card 1");
                g_sdmmc_info1.present = FALSE;
                g_sdmmc_info1.init = TRUE;
                return MCD_ERR_NO_CARD;
            }
...
extern MCD_ERR_T mcd_Init_sdmmc2(MCD_CSD_T* mcdCsd, MCD_CARD_VER mcdVer,MCD_SDMMC_SPI_IF* mcdConfig);


PUBLIC MCD_ERR_T mcd_Open_Dual_sdmmc(MCD_CSD_T* mcdCsd, MCD_CARD_VER mcdVer,MCD_SDMMC_SPI_IF* mcdConfig,UINT8 card_ID)
{
    MCD_ERR_T  errStatus1 = MCD_ERR_NO_CARD,errStatus2=MCD_ERR_NO_CARD,errStatus3;
    if (g_sdmmc_info1.init == FALSE || g_sdmmc_info2.init == FALSE )
    {
        MCD_TRACE(TSTDOUT, 0, "mcd: init the two card"); hal_HstSendEvent(0x7788aa6a);
        errStatus3 = mcd_Init_sdmmc1(&(g_sdmmc_info1.csdinfo), mcdVer,mcdConfig); hal_HstSendEvent(0x7700aa5a); hal_HstSendEvent(errStatus3);
        errStatus3 = mcd_Init_sdmmc2(&(g_sdmmc_info2.csdinfo), mcdVer,mcdConfig); hal_HstSendEvent(0x7701aa5a); hal_HstSendEvent(errStatus3);
        mcd_sdmmc_Close_all_volt();
    }
    MCD_TRACE(TSTDOUT, 0, "mcd1: open two card.");
    if ((g_sdmmc_info1.present == TRUE) && (g_sdmmc_info2.present == TRUE) )
    {
        hal_HstSendEvent(0x7788aa5a);

        if ((g_sdmmc_info1.open == FALSE) || (g_sdmmc_info2.open == FALSE) )
        {
            hal_HstSendEvent(0x7788aa5e);

            mcd_sdmmc_rst_volt();
            MCD_TRACE(TSTDOUT, 0, "mcd1: follow open two card.");
            errStatus1 = mcd_sdmmc_Open_for_two_sdmmc_1(&(g_sdmmc_info1.csdinfo), mcdVer,mcdConfig);
            if(errStatus1 == MCD_ERR_NO)
            {
                g_sdmmc_info1.open = TRUE;
            }
            else
            {
                g_sdmmc_info1.present = FALSE;
            ...
            errStatus2 = mcd_sdmmc_Open_for_two_sdmmc_2(&(g_sdmmc_info2.csdinfo), mcdVer,mcdConfig);
            if(errStatus2 == MCD_ERR_NO)
            {
                g_sdmmc_info2.open = TRUE;
            }
            else
            {
            ...
            if(card_ID == 1)
                *mcdCsd = g_sdmmc_info1.csdinfo;
            else
                *mcdCsd = g_sdmmc_info2.csdinfo;
        }
        else
        {
            hal_HstSendEvent(0x77881a50);
            MCD_TRACE(TSTDOUT, 0, "mcd: already open two card");
            errStatus1 = MCD_ERR_NO; errStatus2 = MCD_ERR_NO;
            if(card_ID == 1)
                *mcdCsd = g_sdmmc_info1.csdinfo;
            else
                *mcdCsd = g_sdmmc_info2.csdinfo;
    ...
    else if (g_sdmmc_info1.present == TRUE)
    {


        if (g_sdmmc_info1.open == FALSE)
        {
            MCD_TRACE(TSTDOUT, 0, "mcd: only init main card"); hal_HstSendEvent(0x7788aa4a);
            mcd_sdmmc_rst_volt();
            errStatus1 = mcd_sdmmc_Open_for_two_sdmmc_1(&(g_sdmmc_info1.csdinfo), mcdVer,mcdConfig);
            if(errStatus1 == MCD_ERR_NO)
            {
        ...
        {
            MCD_TRACE(TSTDOUT, 0, "mcd: main card already open"); hal_HstSendEvent(0x7788aa3a);
            errStatus1 = MCD_ERR_NO; errStatus2 = MCD_ERR_NO_CARD;
            if(card_ID == 1)
                *mcdCsd = g_sdmmc_info1.csdinfo;
            else
                *mcdCsd = g_sdmmc_info2.csdinfo;
    ...
    else if (g_sdmmc_info2.present == TRUE)
    {
        hal_HstSendEvent(0x77881a51);
        if (g_sdmmc_info2.open == FALSE)
        {
            MCD_TRACE(TSTDOUT, 0, "mcd: open Sec card"); hal_HstSendEvent(0x7788aa1b);
            mcd_sdmmc_rst_volt();
            ...
            else
            {
                g_sdmmc_info2.present = FALSE;
                g_sdmmc_info2.open = FALSE;
                hal_HstSendEvent(0x7788a11b);
            }
        }
        else
        {
            MCD_TRACE(TSTDOUT, 0, "mcd2: Sec card already open card_ID=%d",card_ID); hal_HstSendEvent(0x7788aa2a);
            errStatus2 = MCD_ERR_NO; errStatus1 = MCD_ERR_NO_CARD;
            if(card_ID == 1)
                *mcdCsd = g_sdmmc_info1.csdinfo;
            else
                *mcdCsd = g_sdmmc_info2.csdinfo;
    ...
    if(card_ID == 1)
    {
        hal_HstSendEvent(0x7788aaaa);
        MCD_TRACE(TSTDOUT, 0, "mcd:1 return %d",errStatus1);// MCD_ASSERT(FALSE, "ww1");
        return errStatus1;
    }
    if(card_ID == 2)
    {
        hal_HstSendEvent(0x7788aaab);
        MCD_TRACE(TSTDOUT, 0, "mcd:2 return %d",errStatus2);// MCD_ASSERT(FALSE, "www2");
        return errStatus2;
    }
...
{
    MCD_TRACE(TSTDOUT, 0, "mcd1: open main card");
    return mcd_Open_Dual_sdmmc(mcdCsd,mcdVer,mcdConfig,1);
}
...
{
    MCD_TRACE(TSTDOUT, 0, "mcd1: Close");
    ...
    MCD_TRACE(TSTDOUT, 0, "mcd1: Close sleep in ");
    if (g_mcdSemaphore == 0xffffffff)
    {
        MCD_TRACE(TSTDOUT, 0, "mcd_sdmmc_Close: Never opened before");
        return MCD_ERR_NO;
    }
...
// program_right_num=0;
    ...
    {
        MCD_TRACE(TSTDOUT, 0, "card 1 Write block,Card Reponse: %x", cardResponse[0]);
        hal_SdmmcStopTransfer(&g_mcdSdmmcTransfer);
        return MCD_ERR;
    }
    ...
    {
        MCD_TRACE(TSTDOUT, 0, "SDMMC Write error");
        return MCD_ERR;
    }
...
//=============================================================================
// mcd_SdmmcMultBlockRead HAL_SDMMC_CMD_STOP_TRANSMISSION
//-----------------------------------------------------------------------------
...
PRIVATE MCD_ERR_T mcd_SdmmcMultBlockRead(UINT8* blockAddr, UINT8* pRead, UINT32 blockNum)
{
    ...
    if (cardResponse[0] != cardStatusTranState.reg)
    {
        MCD_TRACE(TSTDOUT, 0, "card 1  CMD%d status=0x%x", readCmd,cardResponse[0]);
        hal_HstSendEvent(0xcdcd1134);
        hal_SdmmcStopTransfer(&g_mcdSdmmcTransfer);
        return MCD_ERR;
    }
