...
PRIVATE void mcd_hal_delay(UINT32 tktnum)
{
#if 1
    UINT32 wait_time = 0;
...
PUBLIC MCD_ERR_T mcd_spi_Open(MCD_CSD_T* mcdCsd, MCD_CARD_VER mcdVer,MCD_SDMMC_SPI_IF* mcdConfig)
{
    ...
    MCD_CS_ENTER;
    hal_HstSendEvent(0xaaaabb03);
    if(NULL == g_mcdConfig)
    {
        g_mcdConfig = tgt_GetMcdConfig();
    }

    hal_HstSendEvent(0xaaaabb01);
    ...
    MCD_TRACE(TSTDOUT, 0, " use the sdmmc2 here3 ");
    if (isMmc)
    {
        while(1)
        {
    ...
    MCD_TRACE(TSTDOUT, 0, " use the sdmmc2 here 5 ");
