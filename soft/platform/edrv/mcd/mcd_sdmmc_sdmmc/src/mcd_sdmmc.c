...
PUBLIC MCD_ERR_T mcd_Open1(MCD_CSD_T* mcdCsd, MCD_CARD_VER mcdVer)
{
    MCD_SDMMC_SPI_IF dd;
    mcd_sdmmc_Open(mcdCsd,mcdVer,&dd);
...
{
    MCD_TRACE(TSTDOUT, 0, "mcd_sdmmc_Close");
...
// program_right_num=0;
    ...
    if (blockNum >1)
    {
        ...
        {
            MCD_TRACE(TSTDOUT, 0, "warning ACMD23 status=%0x", cardResponse[0]);
...
PRIVATE MCD_ERR_T mcd_SdmmcMultBlockRead(UINT8* blockAddr, UINT8* pRead, UINT32 blockNum)
{
    ...
    if (cardResponse[0] != cardStatusTranState.reg)
    {
        MCD_TRACE(TSTDOUT, 0, "CMD%d status=%0x", cardResponse[0], readCmd);
        hal_SdmmcStopTransfer(&g_mcdSdmmcTransfer);
        return MCD_ERR;
    }
