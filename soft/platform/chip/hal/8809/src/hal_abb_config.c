...
#if defined(CHIP_DIE_8809E2)
        hal_AbbRegWrite(CODEC_PLL_13H, gc_halAbbCodecPll_13h[HAL_SYS_PLL_MODE_OFF]); // int_mode=0
#else
        hal_AbbRegWrite(CODEC_PLL_13H, gc_halAbbCodecPll_13h); // int_mode=0
#endif
#if defined(CHIP_DIE_8809E2)
        hal_AbbRegWrite(CODEC_PLL_14H, gc_halAbbCodecPll_14h[HAL_SYS_PLL_MODE_OFF]); // sdm_en
#else
...
PUBLIC VOID hal_AudSetFmMode(BOOL isFmMode)
{
    ...
    if (isFmMode)
    {
        ...
        fmModeSetting |= ABB_CNT_ADC_CLK(21);
        //   A. fm_mode osr to 00
...
#ifdef CHIP_DIE_8809E2
    g_halAudCodecDigEn |= ABB_DIG_DAC_CLK_INVERT;
#endif
...
PUBLIC  VOID hal_AudEnableHPPA(BOOL on)
{
    if(on)
    {
        // Not to direct-reg codec DAC
        g_halAbbCodecPowerCtrl &= ~ABB_RESET_PA_HP_REG;
    }
    else
    {
        // Direct-reg to disable codec DAC
    ...
    hal_AbbRegWrite(CODEC_POWER_CTRL, g_halAbbCodecPowerCtrl|ABB_RESET_PA_HP_DR);
}
...
#else
        hal_AbbRegWrite(CODEC_HP_DETECT_44H, g_halAudHpDetectFmRegVals.hpDet_44h);
        hal_AbbRegWrite(CODEC_HP_DETECT_45H, g_halAudHpDetectFmRegVals.hpDet_45h);
        hal_AbbRegWrite(CODEC_HP_DETECT_46H, g_halAudHpDetectFmRegVals.hpDet_46h);
        hal_AbbRegWrite(CODEC_HP_DETECT_47H, g_halAudHpDetectFmRegVals.hpDet_47h);
#endif
...
PROTECTED VOID hal_AudSetCodecMode(HAL_AUD_CODEC_MODE_PARAM_T *modeParam)
{
    ...
    if (selCfg.spkMonoSel || selCfg.spkSel)
    {
        ...
        if (modeParam->spkPaForceOn)
        {
            ldoSetting1 |= ABB_PD_DAC_L_REG|ABB_PD_DAC_R_REG;
        }
        else
        {
            if (!modeParam->spkRightChanSel)
            {
                ldoSetting1 |= ABB_PD_DAC_R_REG;
            }
            if (!modeParam->spkLeftChanSel)
            {
