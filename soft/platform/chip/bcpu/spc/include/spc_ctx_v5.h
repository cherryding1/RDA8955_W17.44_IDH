...
#ifndef _SPC_CTX_V5_H_
#define _SPC_CTX_V5_H_
...
typedef struct
{
    ...
    VOLATILE BOOL                  NBurstSchIfc2Done;            //0x00000433
    VOLATILE UINT8                 rx_slot_nb;                   //0x00000434
    VOLATILE UINT8                 sched_rx_slot_nb;             //0x00000435
    VOLATILE UINT8                 tx_left;                      //0x00000436
    ...
    UINT8                          xcvPdnEnable;                 //0x000004D4
    /// amr frame filter enable flag.
    UINT8                          amrFrmFltEnable;              //0x000004D5
    /// IsSaicEnable: 1: enable. 0: not enable
    BOOL                           IsSaicEnable;                 //0x000004D6
    /// interleaver input buffer pointers
    UINT32*                        ItlvBufRxCtrlSaic;            //0x000004D8
    /// Dummy detection snr threshold.
    UINT8                          DummySnrThreshold;            //0x000004DC
} SPC_CONTEXT_T; //Size : 0x4E0
...
typedef struct
{
    UINT32                         RxIfcBuffer[8*(BB_BURST_ACTIVE_SIZE+2*BB_RX_NBURST_MARGIN)]; //0x00000000
    UINT32                         MonIfcBuffer[MBOX_WINDOWS_QTY*MBOX_MON_WIN_SIZE]; //0x00001380
} SPC_STATIC_BUFFERS_T; //Size : 0x1700

EXPORT PROTECTED UINT32 g_spcStaticBufTabSaic[BB_BURST_TOTAL_SIZE*2 + 4];
EXPORT PROTECTED UINT32 g_spcItlvBufTabSaic[SPC_ITLV_BUFF_RX_CTRL*5];
#else
// ============================================================================
// SPC_STATIC_BUFFERS_T
// -----------------------------------------------------------------------------
