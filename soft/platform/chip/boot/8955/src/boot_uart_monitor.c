...
INLINE VOID boot_UartSetDivider(HAL_UART_BAUD_RATE_T rate)
{
    ...
    UINT32 uartClockDivisor = SYS_CTRL_CFG_UART_NUM(1)|SYS_CTRL_CFG_UART_DENOM(113);//0x71
    ...
    switch(rate)
    {
        ...
        case HAL_UART_BAUD_RATE_115200:
            uartClockDivisor = SYS_CTRL_CFG_UART_NUM(1)|SYS_CTRL_CFG_UART_DENOM(113);//0x71
            break;
    ...
    hwp_sysCtrl->Cfg_Clk_Uart[1] = (uartClockDivisor);
...
{
    //  hwp_configRegs->GPIO_Mode        &=       ~(CFG_REGS_MODE_PIN_UART1_RTS | CFG_REGS_MODE_PIN_UART1_CTS);
    UINT32 iodata;
    iodata = hwp_iomux->pad_GPIO_0_cfg;
    iodata = (iodata&(~IOMUX_PAD_GPIO_0_SEL_MASK))|IOMUX_PAD_GPIO_0_SEL(1);
    hwp_iomux->pad_GPIO_0_cfg = iodata;

    iodata = hwp_iomux->pad_GPIO_1_cfg;
    iodata = (iodata&(~IOMUX_PAD_GPIO_1_SEL_MASK))|IOMUX_PAD_GPIO_1_SEL(1);
    hwp_iomux->pad_GPIO_1_cfg = iodata;

    iodata = hwp_iomux->pad_GPIO_2_cfg;
    iodata = (iodata&(~IOMUX_PAD_GPIO_2_SEL_MASK))|IOMUX_PAD_GPIO_2_SEL(1);
    hwp_iomux->pad_GPIO_2_cfg = iodata;

    iodata = hwp_iomux->pad_GPIO_3_cfg;
    iodata = (iodata&(~IOMUX_PAD_GPIO_3_SEL_MASK))|IOMUX_PAD_GPIO_3_SEL(1);
    hwp_iomux->pad_GPIO_3_cfg = iodata;
    //      Enable the UART, @115200.
    hwp_sysCtrl->Cfg_Clk_Uart[1] = SYS_CTRL_CFG_UART_NUM(1)|SYS_CTRL_CFG_UART_DENOM(113);//0x71   15200


    //hwp_sysCtrl->Cfg_Clk_Uart[1]  = SYS_CTRL_UART_DIVIDER(55)| SYS_CTRL_UART_SEL_PLL_SLOW;
    // Configure the UART.
