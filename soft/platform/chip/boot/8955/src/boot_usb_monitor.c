        ...
        switch(g_BootHostUsbVar.State)
        {
            ...
            case BOOT_HOST_USB_STATE_SXS_ADDR:
                ...
                else
                {
                    ...
                    {
                        if(g_BootHostUsbVar.Cmd.rw != BOOT_HOST_USB_CMD_RW_WRITE)
                        {
                            g_BootHostUsbVar.Addr = KSEG1(g_BootHostUsbVar.Addr+REG_INT_SRAM_BASE);
                        }
                    }
                    else
                    {
                        ...
                        boot_DcacheFlushAddrRange(g_BootHostUsbVar.Addr, g_BootHostUsbVar.Addr+g_BootHostUsbVar.DataSize, FALSE);

                        if(g_BootHostUsbVar.Cmd.rw == BOOT_HOST_USB_CMD_RW_WRITE)
                        {
                            g_BootHostUsbVar.Addr = KSEG1(g_BootHostUsbVar.Addr);
                        }
...
boot_HostUsbInterfaceList[2][2] =
{
    {
        &boot_HostUsbInterface[0],
        0
    },
    {
        &boot_HostUsbInterface[1],
        0
    }
};
...
PRIVATE CONST HAL_USB_CONFIG_DESCRIPTOR_CONST_PTR_T boot_HostUsbConfigList[2][2] =
{
    {
        &boot_HostUsbConfig[0],
        0
    },
    {
        &boot_HostUsbConfig[1],
        0
    }
};
...
{
    {
        ...
        .product      = 0,
        .version      = 0,
        .configList   = (HAL_USB_CONFIG_DESCRIPTOR_T**) boot_HostUsbConfigList[0],
    ...
    },
    {
        ...
        .product      = 0x4d00,
        .version      = 0,
        .configList   = (HAL_USB_CONFIG_DESCRIPTOR_T**) boot_HostUsbConfigList[1],
        .description  = (UINT8*)"Coolsand",
        .serialNumber = (UINT8*)"Host USB",
        .usbMode      = 0
    }
...
{
    return (HAL_USB_DEVICE_DESCRIPTOR_T*) &boot_HostUsbDesc[0];
}
...
{
    return (HAL_USB_DEVICE_DESCRIPTOR_T*) &boot_HostUsbDesc[1];
}
...
{
    BOOT_HOST_USB_HEADER_T*            header;
    UINT8                              frame[10], *pframe;
    UINT32                             startTime = hwp_timer->HWTimer_CurVal;
    pframe = (UINT8*)HAL_SYS_GET_UNCACHED_ADDR(frame);
    header = (BOOT_HOST_USB_HEADER_T*) &frame[1];
    ...
    header->flowId         = 0xFF;
    frame[4]               = 0;
    memcpy(&frame[5], &event, sizeof(UINT32));
    frame[9]               = ~(frame[4]^frame[5]^frame[6]^frame[7]^frame[8]);
    ...
    boot_UsbSend(1, pframe, sizeof(frame), 0);
    while(boot_UsbEpTransfertDone(HAL_USB_EP_DIRECTION_IN(1)) == FALSE)
    {
        if((hwp_timer->HWTimer_CurVal - startTime) >= BOOT_USB_SEND_EVENT_DELAY)
        {
