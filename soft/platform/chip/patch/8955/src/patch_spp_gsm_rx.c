...
extern UINT8  g_mailbox_pal2spc_statWin_rx_Usf_ind;
extern UINT8  g_rx_Usf_Decode_ind;
//extern UINT8 g_rx_Snr_ind;
...
SPAL_INTSRAM_PATCH_UCDATA   UINT8   usfDistance_num0  = 20, distance_max_num0 = 11;
PUBLIC UINT8 SPAL_INTSRAM_PATCH_TEXT  patch_spp_GetUSF(UINT32* BufferIn, UINT8 CS)
{
    ...
    {
        g_rx_Usf_Decode_ind = Usf;
    }
    else
    {
        g_rx_Usf_Decode_ind = 0xFF;
    }

    if (g_mailbox_pal2spc_statWin_rx_Usf_ind == 0xFF)
    {
        return g_rx_Usf_Decode_ind;
    }

    UsfPatternTem = USF_pattern[g_mailbox_pal2spc_statWin_rx_Usf_ind];
    Usf = g_mailbox_pal2spc_statWin_rx_Usf_ind;
    distance = 0;
    for (j = 0; j < nb_bits; j++)
    {
    ...
    if(((usfDistance*usfDistance_num0)>(distance_max*distance_max_num0))&&(distance_max>distance_max1_num))
    {
        return Usf;
    }
    else
...
#endif
SPAL_BBSRAM_PATCH_DATA    INT16 gUsfDistance = (INT16)0x8000;
SPAL_BBSRAM_PATCH_DATA    INT16 gDistance_max = 0;
PUBLIC UINT8 SPAL_INTSRAM_PATCH_TEXT  patch_spp_GetUSF1(UINT32* BufferIn, UINT8 CS)
{
    UINT8 i, j, nb_bits;
    UINT8 Usf = 0;
    UINT8* softBit = (UINT8*)BufferIn;
    ...
    gUsfDistance = usfDistance;
    gDistance_max = distance_max;
    if(((usfDistance*20)>(distance_max*16))&&(distance_max>24)) return Usf;
    else return 0xFF;
}
UINT8 SPAL_INTSRAM_PATCH_UCDATA g_mailbox_pal2spc_rxBandInd = 0xff;
...
if ((g_mailbox_pal2spc_rxBandInd == 0)||(g_mailbox_pal2spc_rxBandInd == 1))
{			
    if ((Res->FOf >25000)||(Res->FOf < -25000))
    {
      g_mailbox_pal2spc_rxBandInd = 0xff;
      return FCCH_INVALID; 
    }
}			
#else
