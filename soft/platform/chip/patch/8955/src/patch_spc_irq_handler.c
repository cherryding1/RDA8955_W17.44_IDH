...
UINT8 SPAL_BBSRAM_PATCH_UCDATA g_RxSaicEnFlag = 0xff;
SPAL_INTSRAM_PATCH_TEXT VOID patch_spc_UpdateSaicStatus(UINT8 snap)
{

	if(MBOX_SAIC_STATE_REQ_GET(g_mailbox.spc2pal.debug[1]))
	{
	 	if((MBOX_SAIC_STATE_STATE_GET(g_mailbox.spc2pal.debug[1]) == TRUE) && (0 == g_mailbox.pal2spc.win[snap].rx.nBurst.bstIdx) )
	 	{
#ifdef CHIP_SAIC_SUPPORTED
 			g_spcCtx->IsSaicEnable = TRUE;
			
#ifdef CHIP_SAIC_ENABLE_STEP2
                    g_sppGlobalParams.IsSaicStep2Enable = TRUE;
#else
                    g_sppGlobalParams.IsSaicStep2Enable = FALSE;
#endif
g_RxSaicEnFlag = 1;
#endif
			
 			g_mailbox.spc2pal.debug[1] |= MBOX_SAIC_STATE_RSP_SET;
	 	}
		else if(MBOX_SAIC_STATE_STATE_GET(g_mailbox.spc2pal.debug[1]) == FALSE) 
		{
			g_spcCtx->IsSaicEnable = FALSE;
			g_sppGlobalParams.IsSaicStep2Enable = FALSE;
			g_mailbox.spc2pal.debug[1] |= MBOX_SAIC_STATE_RSP_SET;
g_RxSaicEnFlag =  0;			
		}
 	...
 	if(MBOX_SAIC_STATE_REQ_GET(g_mailbox.spc2pal.debug[1]) && MBOX_SAIC_STATE_RSP_GET(g_mailbox.spc2pal.debug[1]))
 	{
		g_mailbox.spc2pal.debug[1] = 0;
 	}
...
//*********                     ************//
//******************************************//
 PROTECTED SPAL_INTSRAM_PATCH_TEXT VOID patch_spc_IrqHandler(SPAL_IRQ_STATUS_T status)
{
    ...
    patch_spc_UpdateSaicStatus(snap_nbr);
...
/*                          bb_prog_rx_win                                      */
/********************************************************************************/
VOID SPAL_INTSRAM_PATCH_TEXT patch_bb_prog_rx_win(UINT8 win_type,BOOL swapIQ)
{
    ...
    switch (win_type)
    {
        ...
        case GSM_WIN_TYPE_SCH:
            ...
            if( g_spcCtx->IsSaicEnable)
            {
                spal_RfIfWriteCtrl(g_spcCtx->RfIfCtrl | RF_IF_RX_FIFO_RESET | RF_IF_DIGRF_RX_RATE_TWO);
                spal_IfcParamRf(BB_IFC_END_TC,
                                (UINT32) &(g_spcStaticBufTab.RxIfcBuffer[(BB_BURST_ACTIVE_SIZE+2*BB_RX_NBURST_MARGIN)*2]),
                                0,
                                (2*BB_RX_SCH_MARGIN+BB_BURST_ACTIVE_SIZE)*2,
                                BB_IFC_ENABLE | sbnb_ifc_mode);
            }
            else
            {
