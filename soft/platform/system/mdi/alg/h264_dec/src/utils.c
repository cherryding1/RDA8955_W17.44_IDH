...
{
    if((int)w>0 && (int)h>0 && (w+128)*(uint64_t)(h+128) < (int)INT_MAX/4)
        return 0;
...
int avcodec_default_get_buffer(AVCodecContext *s, AVFrame *pic)
{
    ...
    else
    {
        ...
        for(i=0; i<4 && size[i]; i++)
        {
            ...
            buf->base[i]= av_malloc(size[i]+16+8); //FIXME 16, +8 for GOUDA Y addr 8bytes align,sheen
            if(buf->base[i]==NULL) return -1;
            memset(buf->base[i], 128, size[i]);

            // no edge if EDEG EMU or not planar YUV, we check for PAL8 redundantly to protect against a exploitable bug regression ...
            //ALIGN(buf->base[i],8); for GOUAD Y addr 8bytes align,sheen
            if((s->flags&CODEC_FLAG_EMU_EDGE) || (s->pix_fmt == PIX_FMT_PAL8) || !size[2])
                buf->data[i] = (uint8_t*)ALIGN((unsigned int)buf->base[i],8);
            else
                buf->data[i] = (uint8_t*)(ALIGN((unsigned int)buf->base[i],8) + ALIGN((buf->linesize[i]*EDGE_WIDTH>>v_shift) + (EDGE_WIDTH>>h_shift), STRIDE_ALIGN));
        }
        buf->width  = s->width;
        buf->height = s->height;
...
void avcodec_default_release_buffer(AVCodecContext *s, AVFrame *pic)
{
    ...
    AV_ASSERT(pic->type==FF_BUFFER_TYPE_INTERNAL);
    AV_ASSERT(s->internal_buffer_count);
    ...
    AV_ASSERT(i < s->internal_buffer_count);
    s->internal_buffer_count--;
...
void avcodec_get_context_defaults2(AVCodecContext *s, enum CodecType codec_type)
{
    ...
    // av_opt_set_defaults2(s, flags, flags); sheen
    ...
    //sheen add
//  s->skip_loop_filter =AVDISCARD_NONREF;//AVDISCARD_NONKEY; filter P frame or not
//  s->skip_idct = AVDISCARD_NONREF;
//  s->skip_frame = AVDISCARD_NONREF;
//  s->hurry_up =1;
    //s->lowres=1;
    //sheen add.end
}
...
#endif// MEDIA_VOCVID_SUPPORT
//int tt7,tt8,tt9,tt10; //test sheen
...
//  diag_printf("mult tt %d %d %d ",  tt7*1000/16384,tt8*1000/16384,tt9*1000/16384);
#endif
    return ret;
}
...
extern int init_vlc_done;
//extern VPP_H264_DEC_DATA *vpp_h264_dec_data_set;//[MAX_H264MB_VOC_WORK_NUM]; part voc

// init and get h264 handle
h264handle *h264_decoder_init(void)
{
    int res;
    h264handle *pH=0;
    ...
    //malloc whole decoder memory.
    res=h264_memory_init();
    if(res!=0)return 0;

#if 0//part voc
    //for voc
    vpp_h264_dec_data_set=av_malloc(MAX_H264MB_VOC_WORK_NUM*sizeof(VPP_H264_DEC_DATA) +16);
    if(!vpp_h264_dec_data_set)return 0;
    vpp_h264_dec_data_set=(VPP_H264_DEC_DATA*)(((((UINT32)vpp_h264_dec_data_set)+15)>>4)<<4);//16 byte align
#endif
    pH=(h264handle*)av_malloc(sizeof(h264handle));
    if(!pH)return 0;
    pH->codec_context=0;
    pH->frame_picture=0;

    //init table.(crop, zigzag)
    avcodec_init();

    //malloc AVCodecContext memory.
    pH->codec_context= avcodec_alloc_context();
    if(pH->codec_context==0)return 0;

    //malloc AVFrame memory.
    pH->frame_picture= avcodec_alloc_frame();
    if(pH->frame_picture==0)return 0;

    if(h264_decoder.capabilities&CODEC_CAP_TRUNCATED)
        pH->codec_context->flags|= CODEC_FLAG_TRUNCATED; /* we do not send complete frames */
    ...
    /* open it */
    res=avcodec_open(pH->codec_context, &h264_decoder);
    if(res < 0)return 0;

    {
        H264Context *h = pH->codec_context->priv_data;
        h->s.dsp.idct_permutation_type=1;

        dsputil_init(&h->s.dsp, pH->codec_context);
    }

    return pH;
}

void h264_decoder_close(h264handle *pHandle)
{
    if(!pHandle)return;

    avcodec_close(pHandle->codec_context);
    //av_free(pHandle->codec_context);
    //av_free(pHandle->frame_picture);
    h264_memory_free_all();
}
