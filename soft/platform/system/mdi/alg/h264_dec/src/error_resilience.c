        ...
        for(pass=0; (changed || pass<2) && pass<10; pass++)
        {
            ...
            for(mb_y=0; mb_y<s->mb_height; mb_y++)
            {
                for(mb_x=0; mb_x<s->mb_width; mb_x++)
                {
                    ...
                    AV_ASSERT(!IS_INTRA(s->current_picture.mb_type[mb_xy]));
                    AV_ASSERT(s->last_picture_ptr && s->last_picture_ptr->data[0]);
...
void ff_er_add_slice(MpegEncContext *s, int startx, int starty, int endx, int endy, int status)
{
    ...
    if(status & (AC_ERROR|DC_ERROR|MV_ERROR)) s->error_count= (int)INT_MAX;
    ...
    if(start_xy > 0 && s->avctx->thread_count <= 1 && s->avctx->skip_top*s->mb_width < start_i)
    {
        ...
        if(prev_status != (MV_END|DC_END|AC_END)) s->error_count= (int)INT_MAX;
    }
}
