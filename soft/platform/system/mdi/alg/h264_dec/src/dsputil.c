...
\
        pixels+=line_size;\
        for(i=0; i<h; i+=2){\
            uint32_t a_in= AV_RN32(pixels  );\
            uint32_t b_in= AV_RN32(pixels+1);\
            l1=  (a_in&0x03030303UL)\
               + (b_in&0x03030303UL);\
            h1= ((a_in&0xFCFCFCFCUL)>>2)\
              + ((b_in&0xFCFCFCFCUL)>>2);\
            OP(*((uint32_t*)block), h0+h1+(((l0+l1)>>2)&0x0F0F0F0FUL));\
            pixels+=line_size;\
            block +=line_size;\
            a_in= AV_RN32(pixels  );\
            b_in= AV_RN32(pixels+1);\
            l0=  (a_in&0x03030303UL)\
               + (b_in&0x03030303UL)\
               + 0x02020202UL;\
            h0= ((a_in&0xFCFCFCFCUL)>>2)\
              + ((b_in&0xFCFCFCFCUL)>>2);\
            OP(*((uint32_t*)block), h0+h1+(((l0+l1)>>2)&0x0F0F0F0FUL));\
            pixels+=line_size;\
            block +=line_size;\
...
{
    int i;
    int n=sizeof(DCTELEM)*24;
    unsigned int *p=(unsigned int*)blocks;
    //memset(blocks, 0, sizeof(DCTELEM)*6*64);
    for(i=0; i<n; i++)
    {
        *p++=0;
...
//WRAPPER8_16_SQ(hadamard8_diff8x8_c, hadamard8_diff16_c) sheen
