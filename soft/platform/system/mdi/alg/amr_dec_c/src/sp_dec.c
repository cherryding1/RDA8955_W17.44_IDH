...
static int Post_Process_reset( Post_ProcessState *state )
{
    ...
    {
        diag_printf( "Post_Process_reset: invalid parameter\n" );
        return-1;
    }
    state->y2_hi = 0;
    state->y2_lo = 0;
...
static int Decoder_amr_init( Decoder_amrState **state )
{
    ...
    {
        diag_printf(  "Decoder_amr_init: invalid parameter\n" );
        return-1;
    }
    *state = NULL;

    /* allocate memory */
    if ( ( s = ( Decoder_amrState * ) mmc_MemMalloc( sizeof( Decoder_amrState ) ) ) ==
            NULL )
    {
        diag_printf(  "Decoder_amr_init: can not malloc state structure\n" );
        return-1;
    }

    /* DPlsf_init */
    /* allocate memory */
    if ( ( s->lsfState = ( D_plsfState * ) mmc_MemMalloc( sizeof( D_plsfState ) ) ) ==
            NULL )
    {
        diag_printf(  "DPlsf_init: can not malloc state structure\n" );
        return-1;
    }

    /* ecGainPitchInit */
    /* allocate memory */
    if ( ( s->ec_gain_p_st = ( ec_gain_pitchState * ) mmc_MemMalloc( sizeof(
                                 ec_gain_pitchState ) ) ) == NULL )
    {
        diag_printf(  "ecGainPitchInit: can not malloc state structure\n" );
        return-1;
    }

    /* ecGainCodeInit */
    /* allocate memory */
    if ( ( s->ec_gain_c_st = ( ec_gain_codeState * ) mmc_MemMalloc( sizeof(
                                 ec_gain_codeState ) ) ) == NULL )
    {
        diag_printf(  "ecGainCodeInit: can not malloc state structure\n" );
        return-1;
    }

    /* gcPredInit */
    /* allocate memory */
    if ( ( s->pred_state = ( gc_predState * ) mmc_MemMalloc( sizeof( gc_predState ) ) )
            == NULL )
    {
        diag_printf(  "gcPredInit: can not malloc state structure\n" );
        return-1;
    }

    /* Cb_gain_averageInit */
    /* allocate memory */
    if ( ( s->Cb_gain_averState = ( Cb_gain_averageState * ) mmc_MemMalloc( sizeof(
                                      Cb_gain_averageState ) ) ) == NULL )
    {
        diag_printf(  "Cb_gain_averageInit: can not malloc state structure\n" )
        ;
        return-1;
    }
    ...
    /* lsp_avgInit */
    /* allocate memory */
    if ( ( s->lsp_avg_st = ( lsp_avgState * ) mmc_MemMalloc( sizeof( lsp_avgState ) ) )
            == NULL )
    {
        diag_printf(  "lsp_avgInit: can not malloc state structure\n" );
        return-1;
    }

    /* Bgn_scdInit */
    /* allocate memory */
    if ( ( s->background_state = ( Bgn_scdState * ) mmc_MemMalloc( sizeof( Bgn_scdState
                                                                         ) ) ) == NULL )
    {
        diag_printf(  "Bgn_scdInit: can not malloc state structure\n" );
        return-1;
    }

    /* phDispInit */
    /* allocate memory */
    if ( ( s->ph_disp_st = ( ph_dispState * ) mmc_MemMalloc( sizeof( ph_dispState ) ) )
            == NULL )
    {
        diag_printf(  "phDispInit: can not malloc state structure\n" );
        return-1;
    }

    /* dtxDecInit */
    /* allocate memory */
    if ( ( s->dtxDecoderState = ( dtx_decState * ) mmc_MemMalloc( sizeof( dtx_decState )
                                                                ) ) == NULL )
    {
        diag_printf(  "dtxDecInit: can not malloc state structure\n" );
        return-1;
    }
...
static int Post_Filter_reset( Post_FilterState *state )
{
    ...
    {
        diag_printf(  "Post_Filter_reset: invalid parameter\n" );
        return-1;
    }
...
static int Post_Filter_init( Post_FilterState **state )
{
    ...
    if ( ( Post_FilterState * )state == NULL )
    {
        diag_printf(  "F057:invalid parameter\n" );
        return-1;
    }
    *state = NULL;

    /* allocate memory */
    if ( ( s = ( Post_FilterState * ) mmc_MemMalloc( sizeof( Post_FilterState ) ) ) ==
            NULL )
    {
        diag_printf(  "F057:can not malloc filter structure\n" );
        return-1;
    }
    s->agc_state = NULL;

    /* allocate memory */
    if ( ( s->agc_state = ( agcState * ) mmc_MemMalloc( sizeof( agcState ) ) ) == NULL )
    {
        diag_printf(  "agcInit: can not malloc state structure\n" );
        return-1;
    }
    Post_Filter_reset( s );
    *state = s;
...
static int Post_Process_init( Post_ProcessState **state )
{
    ...
    if ( ( Post_ProcessState * )state == NULL )
    {
        diag_printf(  "Post_Process_init: invalid parameter\n" );
        return-1;
    }
    *state = NULL;

    /* allocate memory */
    if ( ( s = ( Post_ProcessState * ) mmc_MemMalloc( sizeof( Post_ProcessState ) ) ) ==
            NULL )
    {
        diag_printf(  "Post_Process_init: can not malloc state structure\n" );
        return-1;
    }
...
void * Speech_Decode_Frame_init( )
{
    ...
    /* allocate memory */
    if ( ( s = ( Speech_Decode_FrameState * ) mmc_MemMalloc( sizeof(
                   Speech_Decode_FrameState ) ) ) == NULL )
    {
        diag_printf(  "Speech_Decode_Frame_init: can not malloc state "
                      "structure\n" );
        return NULL;
    }
