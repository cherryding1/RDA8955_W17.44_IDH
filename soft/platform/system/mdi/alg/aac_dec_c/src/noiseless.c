...
void DecodeICSInfo(BitStreamInfo *bsi, ICSInfo *icsInfo, int sampRateIdx)
{
    ...
    icsInfo->icsResBit =      AacGetBits(bsi, 1);
    icsInfo->winSequence =    AacGetBits(bsi, 2);
    icsInfo->winShape =       AacGetBits(bsi, 1);
    if (icsInfo->winSequence == 2)
    {
        /* short block */
        icsInfo->maxSFB =     AacGetBits(bsi, 4);
        icsInfo->sfGroup =    AacGetBits(bsi, 7);
    ...
    {
        /* long block */
        icsInfo->maxSFB =               AacGetBits(bsi, 6);
        icsInfo->predictorDataPresent = AacGetBits(bsi, 1);
        if (icsInfo->predictorDataPresent)
        {
            icsInfo->predictorReset =   AacGetBits(bsi, 1);
            if (icsInfo->predictorReset)
                icsInfo->predictorResetGroupNum = AacGetBits(bsi, 5);
            for (sfb = 0; sfb < MIN(icsInfo->maxSFB, predSFBMax[sampRateIdx]); sfb++)
                icsInfo->predictionUsed[sfb] = AacGetBits(bsi, 1);
        }
        icsInfo->numWinGroup = 1;
        icsInfo->winGroupLen[0] = 1;
    }
...
static void DecodeSectionData(BitStreamInfo *bsi, int winSequence, int numWinGrp, int maxSFB, unsigned char *sfbCodeBook)
{
    ...
    {
        sfb = 0;
        while (sfb < maxSFB &&(bsi->nBytes>0 || bsi->cachedBits>0))
        {
            cb = AacGetBits(bsi, 4);    /* next section codebook */
            sectLen = 0;
            do
            {
                sectLenIncr = AacGetBits(bsi, sectLenBits);
                sectLen += sectLenIncr;
            }
...
static void DecodePulseInfo(BitStreamInfo *bsi, PulseInfo *pi)
{
    ...
    pi->numPulse = AacGetBits(bsi, 2) + 1;      /* add 1 here */
    pi->startSFB = AacGetBits(bsi, 6);
    for (i = 0; i < pi->numPulse; i++)
    {
        pi->offset[i] = AacGetBits(bsi, 5);
        pi->amp[i] = AacGetBits(bsi, 4);
    }
}
...
static void DecodeTNSInfo(BitStreamInfo *bsi, int winSequence, TNSInfo *ti, signed char *tnsCoef)
{
    ...
    if (winSequence == 2)
    {
        ...
        {
            ti->numFilt[w] = AacGetBits(bsi, 1);
            if (ti->numFilt[w])
            {
                ti->coefRes[w] = AacGetBits(bsi, 1) + 3;
                *filtLength =    AacGetBits(bsi, 4);
                *filtOrder =     AacGetBits(bsi, 3);
                if (*filtOrder)
                {
                    *filtDir++ =      AacGetBits(bsi, 1);
                    compress =        AacGetBits(bsi, 1);
                    coefBits = (int)ti->coefRes[w] - compress;  /* 2, 3, or 4 */
                    s = sgnMask[coefBits - 2];
                    n = negMask[coefBits - 2];
                    for (i = 0; i < *filtOrder; i++)
                    {
                        c = AacGetBits(bsi, coefBits);
                        if (c & s)  c |= n;
                        *tnsCoef++ = c;
                    }
                }
    ...
    {
        /* long blocks */
        ti->numFilt[0] = AacGetBits(bsi, 2);
        if (ti->numFilt[0])
            ti->coefRes[0] = AacGetBits(bsi, 1) + 3;
        for (f = 0; f < ti->numFilt[0]; f++)
        {
            *filtLength =      AacGetBits(bsi, 6);
            *filtOrder =       AacGetBits(bsi, 5);
            if (*filtOrder)
            {
                *filtDir++ =     AacGetBits(bsi, 1);
                compress =       AacGetBits(bsi, 1);
...
static void DecodeGainControlInfo(BitStreamInfo *bsi, int winSequence, GainControlInfo *gi)
{
    ...
    for (bd = 1; bd <= gi->maxBand; bd++)
    {
        ...
        {
            gi->adjNum[bd][wd] = AacGetBits(bsi, 3);
            for (ad = 0; ad < gi->adjNum[bd][wd]; ad++)
            {
                gi->alevCode[bd][wd][ad] = AacGetBits(bsi, 4);
                gi->alocCode[bd][wd][ad] = AacGetBits(bsi, (wd == 0 ? locBitsZero : locBits));
            }
        }
    }
}
