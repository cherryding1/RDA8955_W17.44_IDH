...
#else
    if((sizeof(short)*2*6*(MBC + 2)*2)>mmc_MemGetFree()-32)
    {
        return -1;
    }
    MV=(short*)mmc_MemMalloc(sizeof(short)*2*6*(MBC + 2)*2);//only use 2 mb line
    if(!MV )
        return -1;
    memset(MV, 0, (sizeof(short)*2*6*(MBC + 2)*2));
#endif
...
#else
    if(sizeof(short)*(MBC + 2)*2>mmc_MemGetFree()-32)
    {
        return -1;
    }
    modemap=(short*)mmc_MemMalloc(sizeof(short)*(MBC + 2)*2);//only need 2 mb line
    if(!modemap )
        return -1;
    memset(modemap, 0, sizeof(short)*(MBC + 2)*2);
#endif
#endif
...
extern void video_voc_reset(void);
/**********************************************************************
 *
 *      Name:         getpicture
    ...
    switch (pict_type)
    {
        ...
        case PCT_INTER:
            ...
            *(vocWorkState.pVOC_VID)=2;//more voc work to do
            ...
            {
                int bitsremain;
                //get parameters for voc
                //bs ptr int align.
                ld->rdptr += (ld->bitcnt)>>3;
                ld->bitcnt = ld->bitcnt & 0x7;
                bitsremain= (4- ((int)ld->rdptr&3))*8- ld->bitcnt;

                *((INT32*) NoCacheAddr(hal_VocGetPointer(H263_G_X_mb_width)))= (INT32)(mb_width|(mb_height<<16));
                *((INT32*) NoCacheAddr(hal_VocGetPointer(H263_G_X_gob)))= (INT32)(gob|(bitsremain<<16));
                *((INT32*) NoCacheAddr(hal_VocGetPointer(H263_G_X_bs_cache)))= (INT32)showbits(bitsremain);
                *((INT32*) NoCacheAddr(hal_VocGetPointer(H263_G_X_bs_buf_extaddr)))= (INT32)(ld->rdptr+ (4- ((INT32)ld->rdptr&3)))&VOC_CFG_DMA_EADDR_MASK;
                *((INT32*) NoCacheAddr(hal_VocGetPointer(H263_G_X_prev_I_P_frame0_extaddr)))= (INT32)prev_I_P_frame[0]&VOC_CFG_DMA_EADDR_MASK;
                *((INT32*) NoCacheAddr(hal_VocGetPointer(H263_G_X_prev_I_P_frame1_extaddr)))= (INT32)prev_I_P_frame[1]&VOC_CFG_DMA_EADDR_MASK;
                *((INT32*) NoCacheAddr(hal_VocGetPointer(H263_G_X_prev_I_P_frame2_extaddr)))= (INT32)prev_I_P_frame[2]&VOC_CFG_DMA_EADDR_MASK;
                *((INT32*) NoCacheAddr(hal_VocGetPointer(H263_G_X_current_frame0_extaddr)))= (INT32)current_frame[0]&VOC_CFG_DMA_EADDR_MASK;
                *((INT32*) NoCacheAddr(hal_VocGetPointer(H263_G_X_current_frame1_extaddr)))= (INT32)current_frame[1]&VOC_CFG_DMA_EADDR_MASK;
                *((INT32*) NoCacheAddr(hal_VocGetPointer(H263_G_X_current_frame2_extaddr)))= (INT32)current_frame[2]&VOC_CFG_DMA_EADDR_MASK;
                *((INT32*) NoCacheAddr(hal_VocGetPointer(H263_G_X_coded_picture_width)))= (INT32)((coded_picture_width)|((coded_picture_height)<<16));
                *((INT32*) NoCacheAddr(hal_VocGetPointer(H263_G_X_pb_frame)))= (INT32)(0|(quant<<16));
                *((INT32*) NoCacheAddr(hal_VocGetPointer(H263_G_X_pict_type)))= (INT32)(pict_type);//hi16=0
                *((INT32*) NoCacheAddr(hal_VocGetPointer(H263_G_X_pgfid)))= (INT32)(pgfid|(UFEP<<16));
                *((INT32*) NoCacheAddr(hal_VocGetPointer(H263_G_X_rtype)))= (INT32)(rtype|(source_format<<16));

                *((INT32*) NoCacheAddr(hal_VocGetPointer(H263_L_X_LOWRES)))= (INT32)(h263_lowres|(0<<16));
            ...
            while(*(vocWorkState.pVOC_VID) !=0)
            {
                //wait last voc vid done
                res=0;
                if((voct1=hal_TimGetUpTime())-voct0>3276)//200ms
                {
                    UINT32 cs_status=0;
                    video_voc_reset();
                    cs_status=hal_SysEnterCriticalSection();
                    *(vocWorkState.pVOC_WORK)=0;
                ...
                if(vocWorkState.aud_nframe_dec)
                    res=vocWorkState.aud_nframe_dec();
                //block display second
                /*if(res!=1 && vocWorkState.vid_display)
                {
                    vocWorkState.vid_display();
                    vocWorkState.vid_display= NULL;
                    //sxr_Sleep(1);
                ...
                // read next frame
                if(res!=1 && vocWorkState.vid_frame_read)
                {
...
#endif
            //get bs consume
            ld->bitcnt= *((INT32*) NoCacheAddr(hal_VocGetPointer(H263_G_X_gob)));
            ld->bitcnt>>=16;
            ld->bitcnt= 32- ld->bitcnt;
            ld->rdptr= ld->rdptr- ((int)ld->rdptr&3) + *((INT32*) NoCacheAddr(hal_VocGetPointer(H263_G_X_bs_buf_extaddr)));//reuse
            //get bs consume end
            errorstate= *((INT16*) NoCacheAddr(hal_VocGetPointer(H263_G_X_errorstate)));
            //diag_printf("err=%d size=%d",errorstate,ld->rdptr- ld->rdbfr);
...
static void get_I_P_MBs (int framenum, int gob)
{
    ...
    */
    ...
    MV_topline= MV;
    MV_curline= MV+ (MBC+2);
    modemap_topline= modemap;
    modemap_curline= modemap +(MBC+2);
    tmp=(MBC+2)*2;
    for (i = 1; i < mb_width + 1; i++)
    {
        for (k = 0; k < 5; k++)
        {
            //MV[0][k][0][i] = NO_VEC;//vector 无矢量
            tmp2=k*tmp+i;
            *(MV_topline+tmp2) = NO_VEC;
            //MV[1][k][0][i] = NO_VEC;
            tmp2+=6*tmp;
            *(MV_topline+tmp2) = NO_VEC;
        }
        //modemap[0][i] = MODE_INTRA;
        *(modemap_topline+i) = MODE_INTRA;
    }
    /* zero MV's on the sides of the picture */
    tmp4=(mb_width + 1);
    for (i = 0; i < 2; i++)
    {
        ...
        //modemap[i][0] = MODE_INTRA;
        tmp2=(MBC+2);
        *(modemap_topline+i*tmp2) = MODE_INTRA;
        //modemap[i][mb_width + 1] = MODE_INTRA;
        *(modemap_topline+i*tmp2+tmp4) = MODE_INTRA;
    }
#endif
        ...
        */
        ...
        if ((ModeBit&0x18)!=0)
        {
            ...
            *(MV+tmp)= *(MV+tmp2) =0;
            ...
            tmp=(xpos + 1);
            tmp2=tmp+6*2*(MBC+2);
            *(MV_curline+tmp)= *(MV_curline+tmp2) =0;
#endif
            //}
        }
        ...
        if (MBA > 0 /*&& !dont_reconstruct_next_mb*/)
        {
...
#else
            if(xpos==0)//exchange addr for Mode and reconstruct
            {
                short *p;
                p= MV_curline;
                MV_curline= MV_topline;
                MV_topline= p;
                p= modemap_curline;
                modemap_curline= modemap_topline;
                modemap_topline= p;
            }
            if(h263_lowres==1)  //add by 20130413
                Mode = *(modemap_curline+((bx>>3) + 1));
            else
                Mode = *(modemap_curline+((bx>>4) + 1));
#endif
            switch(Mode)
            {
                case MODE_INTER:
            ...
            if ((ModeBit&0x18)!=0)
            {
                ...
                for (comp = 0; comp < blk_cnt; comp++)
                {
                    ...
                    if(h263_lowres==1) //add by 4.09
                    {
                        h263_idct16(ld->block+(comp<<6));
                    }
                    else
                    {
#ifdef USE_INTERNAL_SRAM
    ...
    */
    ...
    if (cc == 0)
    {
        ...
        */
        ...
        if(h263_lowres==1)
        {
            rfp = current_frame[0] + (coded_picture_width>>1) * (by + ((comp & 2) << 1)) + bx + ((comp & 1) << 2);
    ...
    else
    {
        ...
        */
        ...
        if(h263_lowres==1)
        {
            rfp = current_frame[cc] + (chrom_width>>1) * by + bx;
            iincr = chrom_width>>1;
        }
        else
        {
    ...
    if(h263_lowres==1)     //add by 410
    {
        if(addflag)
        {
            h263_add_pixels_clamped4_c(rfp,bp,iincr);
        }
        else
        {
            put_h263_pixels_clamped4_c(rfp,bp,iincr);
        }
    }
    else
    {
...
int find_pmv (int x, int y, int block, int comp)
{
    ...
    o8=2;//(MBR+1);
    or8=(MBC+2);
    p1=*(MV1+(l8+vec1)*o8*or8+xin1);
    //p2 = MV[comp][vec2][yin2][xin2];
    p2=*(MV2+(l8+vec2)*o8*or8+xin2);
    //p3 = MV[comp][vec3][yin3][xin3];
    p3=*(MV3+(l8+vec3)*o8*or8+xin3);
...
//move from tmndec.c  shenh
    void initdecoder ()
    {
        ...
        */
        if(h263_lowres==1)
        {
            size = (coded_picture_width >>1) * (coded_picture_height>>1);
        }
        else
            size = coded_picture_width * coded_picture_height;
        if(!pVid_ext_buf)
        {
            ...
            if (!(next_I_P_frame[0] = (unsigned char *) mmc_MemMalloc ((size*3/2)+8)))//next yuv frame will putin. shenh
            {
                //error ("mmc_MemMalloc failed\n");
                errorstate=-1;
                return;
            ...
            next_I_P_frame[0] +=8;
            next_I_P_frame[0] =(unsigned char*)(((unsigned int)next_I_P_frame[0]>>3)<<3);
        }
        else
        {
            next_I_P_frame[0] = (unsigned char *)pVid_ext_buf;
        }
        ...
        if (!(current_frame[0] = (unsigned char *) mmc_MemMalloc ((size*3/2)+8)))//switch address with next_I_P_frame for output yuv.shenh
        {
            //error ("mmc_MemMalloc failed\n");
            errorstate=-1;
            return;
        ...
        //8bytes algin for gouda y addr.
        current_frame[0] +=8;
        current_frame[0] =(unsigned char*)(((unsigned int)current_frame[0]>>3)<<3);
