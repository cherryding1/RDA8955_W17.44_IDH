...
static void getpicturehdr ()
{
    ...
    else
    {
        ...
        mb_width = horizontal_size / 16;
        mb_height = vertical_size / 16;
        if(mb_width*mb_height > (MBC*MBR))//sheen
        {
            diag_printf ("error: mb_width=%d mb_height=%d bigger than MBC,MBR\n", mb_width,mb_height);
            //exit (-1);
            errorstate=-1;
            return;
        }
