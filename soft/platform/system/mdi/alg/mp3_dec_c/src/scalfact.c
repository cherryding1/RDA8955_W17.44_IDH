...
static void UnpackSFMPEG1(BitStreamInfo *bsi, SideInfoSub *sis, ScaleFactorInfoSub *sfis, int *scfsi, int gr, ScaleFactorInfoSub *sfisGr0)
{
    ...
    /* these can be 0, so make sure Mp3GetBits(bsi, 0) returns 0 (no >> 32 or anything) */
    ...
    if (sis->blockType == 2)
    {
        ...
        if (sis->mixedBlock)
        {
            ...
            for (sfb = 0; sfb < 8; sfb++)
                sfis->l[sfb] =    (char)Mp3GetBits(bsi, slen0);
            sfb = 3;
        }
        else
        {
        ...
        for (      ; sfb < 6; sfb++)
        {
            sfis->s[sfb][0] = (char)Mp3GetBits(bsi, slen0);
            sfis->s[sfb][1] = (char)Mp3GetBits(bsi, slen0);
            sfis->s[sfb][2] = (char)Mp3GetBits(bsi, slen0);
        }

        for (      ; sfb < 12; sfb++)
        {
            sfis->s[sfb][0] = (char)Mp3GetBits(bsi, slen1);
            sfis->s[sfb][1] = (char)Mp3GetBits(bsi, slen1);
            sfis->s[sfb][2] = (char)Mp3GetBits(bsi, slen1);
        }
    ...
    else
    {
        ...
        if(gr == 0)
        {
            ...
            for (sfb = 11; sfb < 21; sfb++)
                sfis->l[sfb] = (char)Mp3GetBits(bsi, slen1);
            return;
        }
        else
        {
            ...
            else          for(  ; sfb < 6 ; sfb++) sfis->l[sfb] = (char)Mp3GetBits(bsi, slen0);
            if(scfsi[1])  for(  ; sfb <11 ; sfb++) sfis->l[sfb] = sfisGr0->l[sfb];
            else          for(  ; sfb <11 ; sfb++) sfis->l[sfb] = (char)Mp3GetBits(bsi, slen0);
            if(scfsi[2])  for(  ; sfb <16 ; sfb++) sfis->l[sfb] = sfisGr0->l[sfb];
            else          for(  ; sfb <16 ; sfb++) sfis->l[sfb] = (char)Mp3GetBits(bsi, slen1);
            if(scfsi[3])  for(  ; sfb <21 ; sfb++) sfis->l[sfb] = sfisGr0->l[sfb];
            else          for(  ; sfb <21 ; sfb++) sfis->l[sfb] = (char)Mp3GetBits(bsi, slen1);
        }
        /* last sf band not transmitted */
        sfis->l[21] = 0;
        sfis->l[22] = 0;
...
static void UnpackSFMPEG2(BitStreamInfo *bsi, SideInfoSub *sis, ScaleFactorInfoSub *sfis, int gr, int ch, int modeExt, ScaleFactorJS *sfjs)
{
    ...
    if(sis->blockType == 2)
    {
        if(sis->mixedBlock)
        {
            ...
            for (sfb=0; sfb < 6; sfb++)
            {
                sfis->l[sfb] = (char)Mp3GetBits(bsi, slen[0]);
            }
        ...
        for (    ; nrIdx <= 3; nrIdx++)
        {
            ...
            {
                sfis->s[sfb][0] = (char)Mp3GetBits(bsi, slen[nrIdx]);
                sfis->s[sfb][1] = (char)Mp3GetBits(bsi, slen[nrIdx]);
                sfis->s[sfb][2] = (char)Mp3GetBits(bsi, slen[nrIdx]);
            }
        }
        /* last sf band not transmitted */
        sfis->s[12][0] = sfis->s[12][1] = sfis->s[12][2] = 0;
    ...
    else
    {
        ...
        for (nrIdx = 0; nrIdx <= 3; nrIdx++)
        {
            ...
            for(i=0; i < nr[nrIdx]; i++, sfb++)
            {
                sfis->l[sfb] = (char)Mp3GetBits(bsi, slen[nrIdx]);
            }
        }
        /* last sf band not transmitted */
