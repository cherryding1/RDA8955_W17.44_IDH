...
int UnpackFrameHeader(MP3DecInfo *mp3DecInfo, unsigned char *buf)
{
    ...
    /* read header fields - use bitmasks instead of Mp3GetBits() for speed, since format never varies */
    verIdx =         (buf[1] >> 3) & 0x03;
...
int UnpackSideInfo(MP3DecInfo *mp3DecInfo, unsigned char *buf)
{
    ...
    if (fh->ver == MPEG1)
    {
        ...
        si->mainDataBegin = Mp3GetBits(bsi, 9);
        si->privateBits =   Mp3GetBits(bsi, (fh->sMode == Mono ? 5 : 3));

        for (ch = 0; ch < mp3DecInfo->nChans; ch++)
            for (bd = 0; bd < MAX_SCFBD; bd++)
                si->scfsi[ch][bd] = Mp3GetBits(bsi, 1);
    }
    else
    {
        /* MPEG 2, MPEG 2.5 */
        ...
        SetBitstreamPointer(bsi, nBytes, buf);
        si->mainDataBegin = Mp3GetBits(bsi, 8);
        si->privateBits =   Mp3GetBits(bsi, (fh->sMode == Mono ? 1 : 2));
    }
    ...
    for(gr =0; gr < mp3DecInfo->nGrans; gr++)
    {
        for (ch = 0; ch < mp3DecInfo->nChans; ch++)
        {
            ...
            sis->part23Length =    Mp3GetBits(bsi, 12);
            sis->nBigvals =        Mp3GetBits(bsi, 9);
            sis->globalGain =      Mp3GetBits(bsi, 8);
            sis->sfCompress =      Mp3GetBits(bsi, (fh->ver == MPEG1 ? 4 : 9));
            sis->winSwitchFlag =   Mp3GetBits(bsi, 1);

            if(sis->winSwitchFlag)
            {
                /* this is a start, stop, short, or mixed block */
                sis->blockType =       Mp3GetBits(bsi, 2);      /* 0 = normal, 1 = start, 2 = short, 3 = stop */
                sis->mixedBlock =      Mp3GetBits(bsi, 1);      /* 0 = not mixed, 1 = mixed */
                sis->tableSelect[0] =  Mp3GetBits(bsi, 5);
                sis->tableSelect[1] =  Mp3GetBits(bsi, 5);
                sis->tableSelect[2] =  0;                   /* unused */
                sis->subBlockGain[0] = Mp3GetBits(bsi, 3);
                sis->subBlockGain[1] = Mp3GetBits(bsi, 3);
                sis->subBlockGain[2] = Mp3GetBits(bsi, 3);
            ...
            else
            {
                ...
                sis->tableSelect[0] =  Mp3GetBits(bsi, 5);
                sis->tableSelect[1] =  Mp3GetBits(bsi, 5);
                sis->tableSelect[2] =  Mp3GetBits(bsi, 5);
                sis->region0Count =    Mp3GetBits(bsi, 4);
                sis->region1Count =    Mp3GetBits(bsi, 3);
            }
            sis->preFlag =           (fh->ver == MPEG1 ? Mp3GetBits(bsi, 1) : 0);
            sis->sfactScale =        Mp3GetBits(bsi, 1);
            sis->count1TableSelect = Mp3GetBits(bsi, 1);
        }
    }
