...
extern int  MP3_feed_buffer(unsigned char * pData, unsigned long len);
extern unsigned char *pOutPcm;
extern unsigned long pcmLen;

//unsigned char tempBuf[4*1152];
struct  audio_stats  dongwl_stats;
/*
 * NAME:    synth->frame()
...
void mad_synth_frame(struct mad_synth *synth, struct mad_frame const *frame)
{
    ...
    nch=1;//only output 1 channel for reduce work.sheen
    ...
    tc=synth->pcm.length/2;

#if 1
    if(nch==2)
    {

        iret= audio_pcm_s16le(pOutPcm, synth->pcm.length , synth->pcm.samples[0], synth->pcm.samples[1], &dongwl_stats);
        pcmLen +=iret;
        pOutPcm +=iret;
    }
    else
    {
        iret= audio_pcm_s16le(pOutPcm, synth->pcm.length , synth->pcm.samples[0], 0, &dongwl_stats);
        pcmLen +=iret;
        pOutPcm +=iret;
    }
...
#else
    iret= audio_pcm_s16le(tempBuf, synth->pcm.length , synth->pcm.samples[0], synth->pcm.samples[1], &dongwl_stats);

    MP3_feed_buffer(tempBuf, iret);
#endif
