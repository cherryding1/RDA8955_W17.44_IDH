...
//  memcpy(&xr[18 * sb], &tmp[sb], (576 - 18 * sb) * sizeof(mad_fixed_t));
    memcpy(&xr[18 * sb], &dtmp[index], (576 - 18 * sb) * sizeof(mad_fixed_t));
}
...
*/
static
enum mad_error III_stereo(mad_fixed_t (*xr)[576],
                          struct granule const *granule,
                          struct mad_header *header,
                          unsigned char const *sfbwidth)
{
 ...
 */
// mad_fixed_t dxr[2][576];

//mad_fixed_t xr1[576];
//mad_fixed_t xr2[576];
//extern mad_fixed_t * xr[2];
// mad_fixed_t * dwlxr[2];
extern mad_fixed_t  (*xr)[576];
...
//extern char * main_data_buf;
//extern char * overlap_buf;

//unsigned char LAY3_BUFFER[MAD_BUFFER_MDLEN*4];
//unsigned char LAY3_OVERLAP[1280*4];
extern char * LAY3_BUFFER;
extern char * LAY3_OVERLAP;
...
//  xr=dxr;
    ...
    {
        stream->main_data =(void*)LAY3_BUFFER;// (unsigned char *)main_data_buf;//malloc(MAD_BUFFER_MDLEN);
        if (stream->main_data == 0)
        {
    ...
    if (frame->overlap == 0)
    {
        frame->overlap =(void*)LAY3_OVERLAP;// (mad_fixed_t *)overlap_buf; //calloc(2 * 32 * 18, sizeof(mad_fixed_t));
        if (frame->overlap == 0)
        {
            stream->error = MAD_ERROR_NOMEM;
            return -1;
...

    {
        ...
        header0 = mad_bit_read(&peek, 32);
        if ((header0 & 0xffe60000L) /* syncword | layer */ == 0xffe20000L)
        {
            if (!(header0 & 0x00010000L))  /* protection_bit */
                mad_bit_skip(&peek, 16);  /* crc_check */

            next_md_begin =
                mad_bit_read(&peek, (header0 & 0x00080000L) /* ID */ ? 9 : 8);
        }
