...
APBS_ENC_STREAM_T InfoNes_Stream;
#define REC_FREQ            735
APBS_AUDIO_CFG_T InfoNES_audioCfg;
char HAL_DATA_INTERNAL InfoNes_Wav[REC_FREQ];
char HAL_DATA_INTERNAL InfoNes_WavCache[REC_FREQ*8];
U32 HAL_DATA_INTERNAL InfoNes_WavOut[REC_FREQ*2];

char *InfoNes_WavCacheCurr = NULL;
U8 InfoNes_WavIdx = 0;
U8 InfoNes_ApbsWork = 0;
SND_ITF_T   InfoNes_itf = SND_ITF_LOUD_SPEAKER ;//SND_ITF_RECEIVER;//SND_ITF_LOUD_SPEAKER;//
U32 InfoNES_SoundOutput_SKIP = 0;
U32 *InfoNES_pcmBuf;
U32 InfoNES_pcmBufLen;
...
#ifdef NES_MEM_ALLOC
struct ApuEvent_t *ApuEventQueue = NULL;
#else
...
void HAL_FUNC_INTERNAL InfoNes_play_callback(APBS_STREAM_STATUS_T event)
{
    U32 valid_len = 0;
    switch (event)
    {
        ...
        {
            if (InfoNes_ApbsWork == 1)
            {
                if((INT32)apbs_GetRemain() <= 0)
                {

                    InfoNES_pcmBuf = (U32 *)InfoNes_WavOut;
                    InfoNES_pcmBufLen = (U32)sizeof(InfoNes_WavOut);
                    memset(InfoNes_WavOut,0x0,InfoNES_pcmBufLen);
                    apbs_SetBuffer((U32*)InfoNES_pcmBuf, (U32)InfoNES_pcmBufLen);
                }
                else
                {
                    apbs_GetWriteBuffer( (U32 **)&InfoNES_pcmBuf, (U32 *)&InfoNES_pcmBufLen );
                }

                valid_len = (U32)InfoNes_WavCacheCurr - (U32)InfoNes_WavCache;

                if(valid_len == 0) return;

                InfoNES_pcmBufLen = (InfoNES_pcmBufLen > valid_len) ?valid_len:InfoNES_pcmBufLen ;
                memcpy(InfoNES_pcmBuf,InfoNes_WavCache, InfoNES_pcmBufLen);
                apbs_AddedData((UINT32) InfoNES_pcmBufLen);

                InfoNes_WavCacheCurr = (U32*)((U32)InfoNes_WavCacheCurr - InfoNES_pcmBufLen);
                if(valid_len > InfoNES_pcmBufLen)
                {
                    memcpy(InfoNes_WavCache,InfoNes_WavCache+InfoNES_pcmBufLen,valid_len - InfoNES_pcmBufLen);
                }
                if(InfoNes_WavCacheCurr+REC_FREQ < &InfoNes_WavCache[0]+REC_FREQ*8)
                    InfoNES_SoundOutput_SKIP = 0;
            }
            break;
        }
    }
...
void InfoNES_SoundInit( void )
{
    ...
    InfoNES_audioCfg.spkLevel = AUD_SPK_VOL_10;
#else
    InfoNES_audioCfg.spkLevel = AUD_SPK_VOL_2;
#endif
    InfoNES_audioCfg.filter= NULL;

    InfoNes_Stream.sampleRate = HAL_AIF_FREQ_44100HZ;
    InfoNes_Stream.channelNb = 1;
    InfoNes_Stream.voiceQuality = FALSE;
    InfoNes_Stream.mode =  MCI_PLAY_MODE_PCM;
    InfoNes_Stream.bitPerSample = 8;
    InfoNes_Stream.handler = InfoNes_play_callback;
...
int InfoNES_SoundOpen( int samples_per_sync, int sample_rate )
{
    ...
    sxs_fprintf(1<21, "InfoNES_SoundOpen ff");
    InfoNes_WavIdx= 0;
    InfoNes_ApbsWork = 0;
    InfoNes_WavIdx= 0;
    InfoNes_ApbsWork = 0;
    InfoNES_SoundOutput_SKIP = 0;
    InfoNes_WavCacheCurr = &InfoNes_WavCache[0];
    InfoNES_pcmBufLen = (U32)sizeof(InfoNes_WavOut);
    memset( InfoNes_Wav,0x0,sizeof(InfoNes_Wav ));
    memset( InfoNes_WavCache,0x0,sizeof(InfoNes_WavCache ));
    memset( InfoNes_WavOut,0x0,sizeof(InfoNes_WavOut ));
...
void InfoNES_SoundClose( void )
{
    ...
    InfoNes_WavIdx= 0;
    InfoNes_ApbsWork = 0;
    sxs_fprintf(1<21, "InfoNES_SoundClose ff %d",ret);

}

/* Sound Output 5 Waves - 2 Pulse, 1 Triangle, 1 Noise. 1 DPCM */
void HAL_FUNC_INTERNAL InfoNES_SoundOutput(int samples, BYTE *wave1, BYTE *wave2, BYTE *wave3, BYTE *wave4, BYTE *wave5)
{

    int i = 0;
    int len = (samples > REC_FREQ)?REC_FREQ:samples ;
    int overlen = 0;
    APBS_ERR_T ret = 0;

    if(InfoNES_SoundOutput_SKIP != 0 )
    {
        return;
    }
    if (InfoNes_ApbsWork == 1)
    {
        memset(InfoNes_Wav,0x0,REC_FREQ);
        for(i = 0; i< len; i++)
        {
            InfoNes_Wav[i] = (wave1[i] + wave2[i] + wave3[i] + wave4[i] + wave5[i] )/5 ;
        }
        memcpy(InfoNes_WavCacheCurr,InfoNes_Wav, len);
        InfoNes_WavCacheCurr += len;

        if(InfoNes_WavCacheCurr+REC_FREQ >= InfoNes_WavCache+REC_FREQ*8)
        {
            InfoNES_SoundOutput_SKIP = 1;
        }

    }
    else
    {
        sxs_fprintf(1<<21,"init buf samples %d  \n",samples);
        memcpy((U8*)InfoNes_WavOut+InfoNes_WavIdx*REC_FREQ,InfoNes_Wav,REC_FREQ);
        if(++InfoNes_WavIdx >= 6 )
        {
            InfoNes_WavIdx = 0;
        }
        sxs_fprintf(1<<21,"init buf end %d  \n",InfoNes_WavIdx);
    }

    if((InfoNes_WavIdx == 0) && (InfoNes_ApbsWork==0))
    {
        InfoNes_ApbsWork = 1;
        InfoNES_SoundOutput_SKIP= 0;
        InfoNes_WavCacheCurr = &InfoNes_WavCache[0];
        apbs_SetBuffer(InfoNes_WavOut, sizeof(InfoNes_WavOut));
        apbs_AddedData((UINT32) REC_FREQ*6);

        InfoNes_Stream.startAddress = (U32*)InfoNes_WavOut;
        InfoNes_Stream.length = sizeof(InfoNes_WavOut);
        ret = apbs_Play(audioItf, &InfoNes_Stream, &InfoNES_audioCfg, TRUE);
        sxs_fprintf(1<<21,"apbs_Play ret %d  \n",ret);
    }
}
...
{
    sxs_fprintf(1<<21,"InfoNES_pAPUDone");
    InfoNES_SoundClose();
}

/*
 * End of InfoNES_pAPU.c
 */
