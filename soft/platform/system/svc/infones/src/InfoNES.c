...
extern int InfoNes_mach_status;
extern unsigned char* g_load_from_sram;
/*-------------------------------------------------------------------*/
/*  NES resources                                                    */
/*-------------------------------------------------------------------*/
...
int InfoNES_Load( const char *pszFileName )
{
    ...
    sxs_fprintf(1<<21,"InfoNES InfoNES_ReadRom. ");

    // Read a ROM image in the memory
    if(NULL != g_load_from_sram)
    {
        InfoNES_ReadRomFromSram(g_load_from_sram);
    }
    else
    {
    ...
    sxs_fprintf(1<<21,"InfoNES InfoNES_Reset. ");
...
int InfoNES_Reset()
{
    ...
    // Reset palette table
    InfoNES_MemorySet( PalTable, 0, sizeof( WORD )*32);

    // Reset APU register
    InfoNES_MemorySet( APU_Reg, 0, sizeof( 4096 ));
...
void InfoNES_Cycle()
{
    ...
    if ( ( PPU_R1 & R1_SHOW_SP ) || ( PPU_R1 & R1_SHOW_SCR ) )
        PPU_Addr = PPU_Temp;
#endif
    sxs_fprintf(1<<21, "InfoNES_Cycle" );
    // Emulation loop
    for (;;)
    {
        int nStep;
        ...
        if ( InfoNES_HSync() == -1 )
        {
            sxs_fprintf(1<<21, " InfoNES_HSync return.");
...
void InfoNES_DrawLine()
{
    ...
    if ( PPU_R1 & R1_SHOW_SP )
    {
        ...
        // Reset sprite buffer
        InfoNES_MemorySet( pSprBuf, 0, sizeof(BYTE)*(NES_DISP_WIDTH + 7) );
