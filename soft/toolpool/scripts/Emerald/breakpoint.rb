...
def bsbp(mode,address)
	...
	when /i|I/
		$BB_CTRL.BCpu_Dbg_BKP.BKPT_Mode.w(0)
	when /rw|RW/
		$BB_CTRL.BCpu_Dbg_BKP.BKPT_Mode.w(3)
	when /r|R/
		$BB_CTRL.BCpu_Dbg_BKP.BKPT_Mode.w(1)
	when /w|W/
		$BB_CTRL.BCpu_Dbg_BKP.BKPT_Mode.w(2)
	else
		puts "Use mode \"i\",\"r\",\"w\", or \"rw\""
		return
	end

	$BB_CTRL.BCpu_Dbg_Addr.Breakpoint_address.w(address)
	$BB_CTRL.BCpu_Dbg_BKP.BKPT_En.w(1)
end
