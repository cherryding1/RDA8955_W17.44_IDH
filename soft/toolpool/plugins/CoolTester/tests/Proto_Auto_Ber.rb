...
class Proto_Auto_Ber < UnitTest
    ...
    @@PARAMETERS=[
        ["band","#{@@DEFAULT_BAND}","Band mode : 1 is GSM850, 2 is GSM900, 3 is DCS1800 and 4 is PCS1900"],
    ...
    def BERWaitForMsSyncAndMsConnect(band)
        ...
        if (WaitCMUSignal("SON",TIMEOUT_SIGNALING_ON,band)==true)
            ...
            if(WaitCMUSignal("SYNC",TIMEOUT_MS_SYNC,band)==true)
				...
				#@berCall = Proto_Auto_Call.new( {:timeout=>10, :start=>1, :number=>'112'} ) 
				#@berCall.process
